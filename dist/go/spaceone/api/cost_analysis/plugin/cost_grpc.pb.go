...
type CostClient interface {
	...
	GetData(ctx context.Context, in *GetDataRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[CostsInfo], error)
}
...
func (c *costClient) GetData(ctx context.Context, in *GetDataRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[CostsInfo], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Cost_ServiceDesc.Streams[0], Cost_GetData_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[GetDataRequest, CostsInfo]{ClientStream: stream}
...
type Cost_GetDataClient = grpc.ServerStreamingClient[CostsInfo]
...
type CostServer interface {
	...
	GetData(*GetDataRequest, grpc.ServerStreamingServer[CostsInfo]) error
	mustEmbedUnimplementedCostServer()
}

// UnimplementedCostServer must be embedded to have
...
func (UnimplementedCostServer) GetData(*GetDataRequest, grpc.ServerStreamingServer[CostsInfo]) error {
...
func (UnimplementedCostServer) testEmbeddedByValue()              {}
...
func RegisterCostServer(s grpc.ServiceRegistrar, srv CostServer) {
	// If the following call pancis, it indicates UnimplementedCostServer was
...
func _Cost_GetData_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(CostServer).GetData(m, &grpc.GenericServerStream[GetDataRequest, CostsInfo]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Cost_GetDataServer = grpc.ServerStreamingServer[CostsInfo]
