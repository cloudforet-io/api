// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.34.2
// 	protoc        v3.6.1
// source: spaceone/api/identity/v2/workspace_group.proto

package v2

import (
	v2 "github.com/cloudforet-io/api/dist/go/spaceone/api/core/v2"
	empty "github.com/golang/protobuf/ptypes/empty"
	_struct "github.com/golang/protobuf/ptypes/struct"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type WorkspaceGroupFindRequest_State int32

const (
	WorkspaceGroupFindRequest_NONE     WorkspaceGroupFindRequest_State = 0
	WorkspaceGroupFindRequest_ENABLED  WorkspaceGroupFindRequest_State = 1
	WorkspaceGroupFindRequest_DISABLED WorkspaceGroupFindRequest_State = 2
	WorkspaceGroupFindRequest_PENDING  WorkspaceGroupFindRequest_State = 3
)

// Enum value maps for WorkspaceGroupFindRequest_State.
var (
	WorkspaceGroupFindRequest_State_name = map[int32]string{
		0: "NONE",
		1: "ENABLED",
		2: "DISABLED",
		3: "PENDING",
	}
	WorkspaceGroupFindRequest_State_value = map[string]int32{
		"NONE":     0,
		"ENABLED":  1,
		"DISABLED": 2,
		"PENDING":  3,
	}
)

func (x WorkspaceGroupFindRequest_State) Enum() *WorkspaceGroupFindRequest_State {
	p := new(WorkspaceGroupFindRequest_State)
	*p = x
	return p
}

func (x WorkspaceGroupFindRequest_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (WorkspaceGroupFindRequest_State) Descriptor() protoreflect.EnumDescriptor {
	return file_spaceone_api_identity_v2_workspace_group_proto_enumTypes[0].Descriptor()
}

func (WorkspaceGroupFindRequest_State) Type() protoreflect.EnumType {
	return &file_spaceone_api_identity_v2_workspace_group_proto_enumTypes[0]
}

func (x WorkspaceGroupFindRequest_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use WorkspaceGroupFindRequest_State.Descriptor instead.
func (WorkspaceGroupFindRequest_State) EnumDescriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{3, 0}
}

type WorkspaceGroupUserSummaryInfo_State int32

const (
	WorkspaceGroupUserSummaryInfo_STATE_NONE WorkspaceGroupUserSummaryInfo_State = 0
	WorkspaceGroupUserSummaryInfo_ENABLED    WorkspaceGroupUserSummaryInfo_State = 1
	WorkspaceGroupUserSummaryInfo_DISABLED   WorkspaceGroupUserSummaryInfo_State = 2
	WorkspaceGroupUserSummaryInfo_PENDING    WorkspaceGroupUserSummaryInfo_State = 3
)

// Enum value maps for WorkspaceGroupUserSummaryInfo_State.
var (
	WorkspaceGroupUserSummaryInfo_State_name = map[int32]string{
		0: "STATE_NONE",
		1: "ENABLED",
		2: "DISABLED",
		3: "PENDING",
	}
	WorkspaceGroupUserSummaryInfo_State_value = map[string]int32{
		"STATE_NONE": 0,
		"ENABLED":    1,
		"DISABLED":   2,
		"PENDING":    3,
	}
)

func (x WorkspaceGroupUserSummaryInfo_State) Enum() *WorkspaceGroupUserSummaryInfo_State {
	p := new(WorkspaceGroupUserSummaryInfo_State)
	*p = x
	return p
}

func (x WorkspaceGroupUserSummaryInfo_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (WorkspaceGroupUserSummaryInfo_State) Descriptor() protoreflect.EnumDescriptor {
	return file_spaceone_api_identity_v2_workspace_group_proto_enumTypes[1].Descriptor()
}

func (WorkspaceGroupUserSummaryInfo_State) Type() protoreflect.EnumType {
	return &file_spaceone_api_identity_v2_workspace_group_proto_enumTypes[1]
}

func (x WorkspaceGroupUserSummaryInfo_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use WorkspaceGroupUserSummaryInfo_State.Descriptor instead.
func (WorkspaceGroupUserSummaryInfo_State) EnumDescriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{11, 0}
}

type CreateWorkspaceGroupRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// +optional
	Tags *_struct.Struct `protobuf:"bytes,2,opt,name=tags,proto3" json:"tags,omitempty"`
}

func (x *CreateWorkspaceGroupRequest) Reset() {
	*x = CreateWorkspaceGroupRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateWorkspaceGroupRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateWorkspaceGroupRequest) ProtoMessage() {}

func (x *CreateWorkspaceGroupRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateWorkspaceGroupRequest.ProtoReflect.Descriptor instead.
func (*CreateWorkspaceGroupRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{0}
}

func (x *CreateWorkspaceGroupRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateWorkspaceGroupRequest) GetTags() *_struct.Struct {
	if x != nil {
		return x.Tags
	}
	return nil
}

type UpdateWorkspaceGroupRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	WorkspaceGroupId string `protobuf:"bytes,1,opt,name=workspace_group_id,json=workspaceGroupId,proto3" json:"workspace_group_id,omitempty"`
	// +optional
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// +optional
	Tags *_struct.Struct `protobuf:"bytes,3,opt,name=tags,proto3" json:"tags,omitempty"`
}

func (x *UpdateWorkspaceGroupRequest) Reset() {
	*x = UpdateWorkspaceGroupRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateWorkspaceGroupRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateWorkspaceGroupRequest) ProtoMessage() {}

func (x *UpdateWorkspaceGroupRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateWorkspaceGroupRequest.ProtoReflect.Descriptor instead.
func (*UpdateWorkspaceGroupRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{1}
}

func (x *UpdateWorkspaceGroupRequest) GetWorkspaceGroupId() string {
	if x != nil {
		return x.WorkspaceGroupId
	}
	return ""
}

func (x *UpdateWorkspaceGroupRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpdateWorkspaceGroupRequest) GetTags() *_struct.Struct {
	if x != nil {
		return x.Tags
	}
	return nil
}

type WorkspacesWorkspaceGroupRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	WorkspaceGroupId string   `protobuf:"bytes,1,opt,name=workspace_group_id,json=workspaceGroupId,proto3" json:"workspace_group_id,omitempty"`
	Workspaces       []string `protobuf:"bytes,2,rep,name=workspaces,proto3" json:"workspaces,omitempty"`
}

func (x *WorkspacesWorkspaceGroupRequest) Reset() {
	*x = WorkspacesWorkspaceGroupRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspacesWorkspaceGroupRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspacesWorkspaceGroupRequest) ProtoMessage() {}

func (x *WorkspacesWorkspaceGroupRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspacesWorkspaceGroupRequest.ProtoReflect.Descriptor instead.
func (*WorkspacesWorkspaceGroupRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{2}
}

func (x *WorkspacesWorkspaceGroupRequest) GetWorkspaceGroupId() string {
	if x != nil {
		return x.WorkspaceGroupId
	}
	return ""
}

func (x *WorkspacesWorkspaceGroupRequest) GetWorkspaces() []string {
	if x != nil {
		return x.Workspaces
	}
	return nil
}

type WorkspaceGroupFindRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Keyword string `protobuf:"bytes,1,opt,name=keyword,proto3" json:"keyword,omitempty"`
	// +optional
	State WorkspaceGroupFindRequest_State `protobuf:"varint,2,opt,name=state,proto3,enum=spaceone.api.identity.v2.WorkspaceGroupFindRequest_State" json:"state,omitempty"`
	// +optional
	Page             *v2.Page `protobuf:"bytes,3,opt,name=page,proto3" json:"page,omitempty"`
	WorkspaceGroupId string   `protobuf:"bytes,21,opt,name=workspace_group_id,json=workspaceGroupId,proto3" json:"workspace_group_id,omitempty"`
}

func (x *WorkspaceGroupFindRequest) Reset() {
	*x = WorkspaceGroupFindRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspaceGroupFindRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspaceGroupFindRequest) ProtoMessage() {}

func (x *WorkspaceGroupFindRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspaceGroupFindRequest.ProtoReflect.Descriptor instead.
func (*WorkspaceGroupFindRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{3}
}

func (x *WorkspaceGroupFindRequest) GetKeyword() string {
	if x != nil {
		return x.Keyword
	}
	return ""
}

func (x *WorkspaceGroupFindRequest) GetState() WorkspaceGroupFindRequest_State {
	if x != nil {
		return x.State
	}
	return WorkspaceGroupFindRequest_NONE
}

func (x *WorkspaceGroupFindRequest) GetPage() *v2.Page {
	if x != nil {
		return x.Page
	}
	return nil
}

func (x *WorkspaceGroupFindRequest) GetWorkspaceGroupId() string {
	if x != nil {
		return x.WorkspaceGroupId
	}
	return ""
}

type AddUsersWorkspaceGroupRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	WorkspaceGroupId string                `protobuf:"bytes,1,opt,name=workspace_group_id,json=workspaceGroupId,proto3" json:"workspace_group_id,omitempty"`
	Users            []*WorkspaceGroupUser `protobuf:"bytes,2,rep,name=users,proto3" json:"users,omitempty"`
}

func (x *AddUsersWorkspaceGroupRequest) Reset() {
	*x = AddUsersWorkspaceGroupRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *AddUsersWorkspaceGroupRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddUsersWorkspaceGroupRequest) ProtoMessage() {}

func (x *AddUsersWorkspaceGroupRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddUsersWorkspaceGroupRequest.ProtoReflect.Descriptor instead.
func (*AddUsersWorkspaceGroupRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{4}
}

func (x *AddUsersWorkspaceGroupRequest) GetWorkspaceGroupId() string {
	if x != nil {
		return x.WorkspaceGroupId
	}
	return ""
}

func (x *AddUsersWorkspaceGroupRequest) GetUsers() []*WorkspaceGroupUser {
	if x != nil {
		return x.Users
	}
	return nil
}

type RemoveUsersWorkspaceGroupRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	WorkspaceGroupId string   `protobuf:"bytes,1,opt,name=workspace_group_id,json=workspaceGroupId,proto3" json:"workspace_group_id,omitempty"`
	Users            []string `protobuf:"bytes,2,rep,name=users,proto3" json:"users,omitempty"`
}

func (x *RemoveUsersWorkspaceGroupRequest) Reset() {
	*x = RemoveUsersWorkspaceGroupRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *RemoveUsersWorkspaceGroupRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveUsersWorkspaceGroupRequest) ProtoMessage() {}

func (x *RemoveUsersWorkspaceGroupRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveUsersWorkspaceGroupRequest.ProtoReflect.Descriptor instead.
func (*RemoveUsersWorkspaceGroupRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{5}
}

func (x *RemoveUsersWorkspaceGroupRequest) GetWorkspaceGroupId() string {
	if x != nil {
		return x.WorkspaceGroupId
	}
	return ""
}

func (x *RemoveUsersWorkspaceGroupRequest) GetUsers() []string {
	if x != nil {
		return x.Users
	}
	return nil
}

type WorkspaceGroupRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	WorkspaceGroupId string `protobuf:"bytes,1,opt,name=workspace_group_id,json=workspaceGroupId,proto3" json:"workspace_group_id,omitempty"`
}

func (x *WorkspaceGroupRequest) Reset() {
	*x = WorkspaceGroupRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspaceGroupRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspaceGroupRequest) ProtoMessage() {}

func (x *WorkspaceGroupRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspaceGroupRequest.ProtoReflect.Descriptor instead.
func (*WorkspaceGroupRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{6}
}

func (x *WorkspaceGroupRequest) GetWorkspaceGroupId() string {
	if x != nil {
		return x.WorkspaceGroupId
	}
	return ""
}

type WorkspaceGroupUser struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	UserId   string `protobuf:"bytes,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	RoleId   string `protobuf:"bytes,2,opt,name=role_id,json=roleId,proto3" json:"role_id,omitempty"`
	RoleType string `protobuf:"bytes,3,opt,name=role_type,json=roleType,proto3" json:"role_type,omitempty"`
}

func (x *WorkspaceGroupUser) Reset() {
	*x = WorkspaceGroupUser{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspaceGroupUser) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspaceGroupUser) ProtoMessage() {}

func (x *WorkspaceGroupUser) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspaceGroupUser.ProtoReflect.Descriptor instead.
func (*WorkspaceGroupUser) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{7}
}

func (x *WorkspaceGroupUser) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *WorkspaceGroupUser) GetRoleId() string {
	if x != nil {
		return x.RoleId
	}
	return ""
}

func (x *WorkspaceGroupUser) GetRoleType() string {
	if x != nil {
		return x.RoleType
	}
	return ""
}

type WorkspaceGroupInfo struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	WorkspaceGroupId string          `protobuf:"bytes,1,opt,name=workspace_group_id,json=workspaceGroupId,proto3" json:"workspace_group_id,omitempty"`
	Name             string          `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Workspaces       []string        `protobuf:"bytes,3,rep,name=workspaces,proto3" json:"workspaces,omitempty"`
	Users            []string        `protobuf:"bytes,4,rep,name=users,proto3" json:"users,omitempty"`
	Tags             *_struct.Struct `protobuf:"bytes,5,opt,name=tags,proto3" json:"tags,omitempty"`
	CreatedBy        string          `protobuf:"bytes,6,opt,name=created_by,json=createdBy,proto3" json:"created_by,omitempty"`
	UpdatedBy        string          `protobuf:"bytes,7,opt,name=updated_by,json=updatedBy,proto3" json:"updated_by,omitempty"`
	DomainId         string          `protobuf:"bytes,21,opt,name=domain_id,json=domainId,proto3" json:"domain_id,omitempty"`
	CreatedAt        string          `protobuf:"bytes,31,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	UpdatedAt        string          `protobuf:"bytes,32,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
}

func (x *WorkspaceGroupInfo) Reset() {
	*x = WorkspaceGroupInfo{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspaceGroupInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspaceGroupInfo) ProtoMessage() {}

func (x *WorkspaceGroupInfo) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspaceGroupInfo.ProtoReflect.Descriptor instead.
func (*WorkspaceGroupInfo) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{8}
}

func (x *WorkspaceGroupInfo) GetWorkspaceGroupId() string {
	if x != nil {
		return x.WorkspaceGroupId
	}
	return ""
}

func (x *WorkspaceGroupInfo) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *WorkspaceGroupInfo) GetWorkspaces() []string {
	if x != nil {
		return x.Workspaces
	}
	return nil
}

func (x *WorkspaceGroupInfo) GetUsers() []string {
	if x != nil {
		return x.Users
	}
	return nil
}

func (x *WorkspaceGroupInfo) GetTags() *_struct.Struct {
	if x != nil {
		return x.Tags
	}
	return nil
}

func (x *WorkspaceGroupInfo) GetCreatedBy() string {
	if x != nil {
		return x.CreatedBy
	}
	return ""
}

func (x *WorkspaceGroupInfo) GetUpdatedBy() string {
	if x != nil {
		return x.UpdatedBy
	}
	return ""
}

func (x *WorkspaceGroupInfo) GetDomainId() string {
	if x != nil {
		return x.DomainId
	}
	return ""
}

func (x *WorkspaceGroupInfo) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

func (x *WorkspaceGroupInfo) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

type WorkspaceGroupSearchQuery struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// +optional
	Query *v2.Query `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	// +optional
	WorkspaceGroupId string `protobuf:"bytes,2,opt,name=workspace_group_id,json=workspaceGroupId,proto3" json:"workspace_group_id,omitempty"`
	// +optional
	Name string `protobuf:"bytes,3,opt,name=name,proto3" json:"name,omitempty"`
	// +optional
	CreatedBy string `protobuf:"bytes,4,opt,name=created_by,json=createdBy,proto3" json:"created_by,omitempty"`
	// +optional
	UpdatedBy string `protobuf:"bytes,5,opt,name=updated_by,json=updatedBy,proto3" json:"updated_by,omitempty"`
}

func (x *WorkspaceGroupSearchQuery) Reset() {
	*x = WorkspaceGroupSearchQuery{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[9]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspaceGroupSearchQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspaceGroupSearchQuery) ProtoMessage() {}

func (x *WorkspaceGroupSearchQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[9]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspaceGroupSearchQuery.ProtoReflect.Descriptor instead.
func (*WorkspaceGroupSearchQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{9}
}

func (x *WorkspaceGroupSearchQuery) GetQuery() *v2.Query {
	if x != nil {
		return x.Query
	}
	return nil
}

func (x *WorkspaceGroupSearchQuery) GetWorkspaceGroupId() string {
	if x != nil {
		return x.WorkspaceGroupId
	}
	return ""
}

func (x *WorkspaceGroupSearchQuery) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *WorkspaceGroupSearchQuery) GetCreatedBy() string {
	if x != nil {
		return x.CreatedBy
	}
	return ""
}

func (x *WorkspaceGroupSearchQuery) GetUpdatedBy() string {
	if x != nil {
		return x.UpdatedBy
	}
	return ""
}

type WorkspaceGroupsInfo struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Results    []*WorkspaceGroupInfo `protobuf:"bytes,1,rep,name=results,proto3" json:"results,omitempty"`
	TotalCount int32                 `protobuf:"varint,2,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
}

func (x *WorkspaceGroupsInfo) Reset() {
	*x = WorkspaceGroupsInfo{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[10]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspaceGroupsInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspaceGroupsInfo) ProtoMessage() {}

func (x *WorkspaceGroupsInfo) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[10]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspaceGroupsInfo.ProtoReflect.Descriptor instead.
func (*WorkspaceGroupsInfo) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{10}
}

func (x *WorkspaceGroupsInfo) GetResults() []*WorkspaceGroupInfo {
	if x != nil {
		return x.Results
	}
	return nil
}

func (x *WorkspaceGroupsInfo) GetTotalCount() int32 {
	if x != nil {
		return x.TotalCount
	}
	return 0
}

type WorkspaceGroupUserSummaryInfo struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	UserId string                              `protobuf:"bytes,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	Name   string                              `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	State  WorkspaceGroupUserSummaryInfo_State `protobuf:"varint,3,opt,name=state,proto3,enum=spaceone.api.identity.v2.WorkspaceGroupUserSummaryInfo_State" json:"state,omitempty"`
}

func (x *WorkspaceGroupUserSummaryInfo) Reset() {
	*x = WorkspaceGroupUserSummaryInfo{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[11]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspaceGroupUserSummaryInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspaceGroupUserSummaryInfo) ProtoMessage() {}

func (x *WorkspaceGroupUserSummaryInfo) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[11]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspaceGroupUserSummaryInfo.ProtoReflect.Descriptor instead.
func (*WorkspaceGroupUserSummaryInfo) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{11}
}

func (x *WorkspaceGroupUserSummaryInfo) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *WorkspaceGroupUserSummaryInfo) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *WorkspaceGroupUserSummaryInfo) GetState() WorkspaceGroupUserSummaryInfo_State {
	if x != nil {
		return x.State
	}
	return WorkspaceGroupUserSummaryInfo_STATE_NONE
}

type WorkspaceGroupUsersSummaryInfo struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Results    []*WorkspaceGroupUserSummaryInfo `protobuf:"bytes,1,rep,name=results,proto3" json:"results,omitempty"`
	TotalCount int32                            `protobuf:"varint,2,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
}

func (x *WorkspaceGroupUsersSummaryInfo) Reset() {
	*x = WorkspaceGroupUsersSummaryInfo{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[12]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspaceGroupUsersSummaryInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspaceGroupUsersSummaryInfo) ProtoMessage() {}

func (x *WorkspaceGroupUsersSummaryInfo) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[12]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspaceGroupUsersSummaryInfo.ProtoReflect.Descriptor instead.
func (*WorkspaceGroupUsersSummaryInfo) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{12}
}

func (x *WorkspaceGroupUsersSummaryInfo) GetResults() []*WorkspaceGroupUserSummaryInfo {
	if x != nil {
		return x.Results
	}
	return nil
}

func (x *WorkspaceGroupUsersSummaryInfo) GetTotalCount() int32 {
	if x != nil {
		return x.TotalCount
	}
	return 0
}

type WorkspaceGroupStatQuery struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Query *v2.StatisticsQuery `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
}

func (x *WorkspaceGroupStatQuery) Reset() {
	*x = WorkspaceGroupStatQuery{}
	if protoimpl.UnsafeEnabled {
		mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[13]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *WorkspaceGroupStatQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkspaceGroupStatQuery) ProtoMessage() {}

func (x *WorkspaceGroupStatQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[13]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkspaceGroupStatQuery.ProtoReflect.Descriptor instead.
func (*WorkspaceGroupStatQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP(), []int{13}
}

func (x *WorkspaceGroupStatQuery) GetQuery() *v2.StatisticsQuery {
	if x != nil {
		return x.Query
	}
	return nil
}

var File_spaceone_api_identity_v2_workspace_group_proto protoreflect.FileDescriptor

var file_spaceone_api_identity_v2_workspace_group_proto_rawDesc = []byte{
	,
}

var (
	file_spaceone_api_identity_v2_workspace_group_proto_rawDescOnce sync.Once
	file_spaceone_api_identity_v2_workspace_group_proto_rawDescData = file_spaceone_api_identity_v2_workspace_group_proto_rawDesc
)

func file_spaceone_api_identity_v2_workspace_group_proto_rawDescGZIP() []byte {
	file_spaceone_api_identity_v2_workspace_group_proto_rawDescOnce.Do(func() {
		file_spaceone_api_identity_v2_workspace_group_proto_rawDescData = protoimpl.X.CompressGZIP(file_spaceone_api_identity_v2_workspace_group_proto_rawDescData)
	})
	return file_spaceone_api_identity_v2_workspace_group_proto_rawDescData
}

var file_spaceone_api_identity_v2_workspace_group_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_spaceone_api_identity_v2_workspace_group_proto_msgTypes = make([]protoimpl.MessageInfo, 14)
var file_spaceone_api_identity_v2_workspace_group_proto_goTypes = []any{
	(WorkspaceGroupFindRequest_State)(0),     // 0: spaceone.api.identity.v2.WorkspaceGroupFindRequest.State
	(WorkspaceGroupUserSummaryInfo_State)(0), // 1: spaceone.api.identity.v2.WorkspaceGroupUserSummaryInfo.State
	(*CreateWorkspaceGroupRequest)(nil),      // 2: spaceone.api.identity.v2.CreateWorkspaceGroupRequest
	(*UpdateWorkspaceGroupRequest)(nil),      // 3: spaceone.api.identity.v2.UpdateWorkspaceGroupRequest
	(*WorkspacesWorkspaceGroupRequest)(nil),  // 4: spaceone.api.identity.v2.WorkspacesWorkspaceGroupRequest
	(*WorkspaceGroupFindRequest)(nil),        // 5: spaceone.api.identity.v2.WorkspaceGroupFindRequest
	(*AddUsersWorkspaceGroupRequest)(nil),    // 6: spaceone.api.identity.v2.AddUsersWorkspaceGroupRequest
	(*RemoveUsersWorkspaceGroupRequest)(nil), // 7: spaceone.api.identity.v2.RemoveUsersWorkspaceGroupRequest
	(*WorkspaceGroupRequest)(nil),            // 8: spaceone.api.identity.v2.WorkspaceGroupRequest
	(*WorkspaceGroupUser)(nil),               // 9: spaceone.api.identity.v2.WorkspaceGroupUser
	(*WorkspaceGroupInfo)(nil),               // 10: spaceone.api.identity.v2.WorkspaceGroupInfo
	(*WorkspaceGroupSearchQuery)(nil),        // 11: spaceone.api.identity.v2.WorkspaceGroupSearchQuery
	(*WorkspaceGroupsInfo)(nil),              // 12: spaceone.api.identity.v2.WorkspaceGroupsInfo
	(*WorkspaceGroupUserSummaryInfo)(nil),    // 13: spaceone.api.identity.v2.WorkspaceGroupUserSummaryInfo
	(*WorkspaceGroupUsersSummaryInfo)(nil),   // 14: spaceone.api.identity.v2.WorkspaceGroupUsersSummaryInfo
	(*WorkspaceGroupStatQuery)(nil),          // 15: spaceone.api.identity.v2.WorkspaceGroupStatQuery
	(*_struct.Struct)(nil),                   // 16: google.protobuf.Struct
	(*v2.Page)(nil),                          // 17: spaceone.api.core.v2.Page
	(*v2.Query)(nil),                         // 18: spaceone.api.core.v2.Query
	(*v2.StatisticsQuery)(nil),               // 19: spaceone.api.core.v2.StatisticsQuery
	(*empty.Empty)(nil),                      // 20: google.protobuf.Empty
}
var file_spaceone_api_identity_v2_workspace_group_proto_depIdxs = []int32{
	16, // 0: spaceone.api.identity.v2.CreateWorkspaceGroupRequest.tags:type_name -> google.protobuf.Struct
	16, // 1: spaceone.api.identity.v2.UpdateWorkspaceGroupRequest.tags:type_name -> google.protobuf.Struct
	0,  // 2: spaceone.api.identity.v2.WorkspaceGroupFindRequest.state:type_name -> spaceone.api.identity.v2.WorkspaceGroupFindRequest.State
	17, // 3: spaceone.api.identity.v2.WorkspaceGroupFindRequest.page:type_name -> spaceone.api.core.v2.Page
	9,  // 4: spaceone.api.identity.v2.AddUsersWorkspaceGroupRequest.users:type_name -> spaceone.api.identity.v2.WorkspaceGroupUser
	16, // 5: spaceone.api.identity.v2.WorkspaceGroupInfo.tags:type_name -> google.protobuf.Struct
	18, // 6: spaceone.api.identity.v2.WorkspaceGroupSearchQuery.query:type_name -> spaceone.api.core.v2.Query
	10, // 7: spaceone.api.identity.v2.WorkspaceGroupsInfo.results:type_name -> spaceone.api.identity.v2.WorkspaceGroupInfo
	1,  // 8: spaceone.api.identity.v2.WorkspaceGroupUserSummaryInfo.state:type_name -> spaceone.api.identity.v2.WorkspaceGroupUserSummaryInfo.State
	13, // 9: spaceone.api.identity.v2.WorkspaceGroupUsersSummaryInfo.results:type_name -> spaceone.api.identity.v2.WorkspaceGroupUserSummaryInfo
	19, // 10: spaceone.api.identity.v2.WorkspaceGroupStatQuery.query:type_name -> spaceone.api.core.v2.StatisticsQuery
	2,  // 11: spaceone.api.identity.v2.WorkspaceGroup.create:input_type -> spaceone.api.identity.v2.CreateWorkspaceGroupRequest
	3,  // 12: spaceone.api.identity.v2.WorkspaceGroup.update:input_type -> spaceone.api.identity.v2.UpdateWorkspaceGroupRequest
	8,  // 13: spaceone.api.identity.v2.WorkspaceGroup.delete:input_type -> spaceone.api.identity.v2.WorkspaceGroupRequest
	4,  // 14: spaceone.api.identity.v2.WorkspaceGroup.add_workspaces:input_type -> spaceone.api.identity.v2.WorkspacesWorkspaceGroupRequest
	4,  // 15: spaceone.api.identity.v2.WorkspaceGroup.remove_workspaces:input_type -> spaceone.api.identity.v2.WorkspacesWorkspaceGroupRequest
	5,  // 16: spaceone.api.identity.v2.WorkspaceGroup.find_users:input_type -> spaceone.api.identity.v2.WorkspaceGroupFindRequest
	6,  // 17: spaceone.api.identity.v2.WorkspaceGroup.add_users:input_type -> spaceone.api.identity.v2.AddUsersWorkspaceGroupRequest
	7,  // 18: spaceone.api.identity.v2.WorkspaceGroup.remove_users:input_type -> spaceone.api.identity.v2.RemoveUsersWorkspaceGroupRequest
	8,  // 19: spaceone.api.identity.v2.WorkspaceGroup.get:input_type -> spaceone.api.identity.v2.WorkspaceGroupRequest
	11, // 20: spaceone.api.identity.v2.WorkspaceGroup.list:input_type -> spaceone.api.identity.v2.WorkspaceGroupSearchQuery
	15, // 21: spaceone.api.identity.v2.WorkspaceGroup.stat:input_type -> spaceone.api.identity.v2.WorkspaceGroupStatQuery
	10, // 22: spaceone.api.identity.v2.WorkspaceGroup.create:output_type -> spaceone.api.identity.v2.WorkspaceGroupInfo
	10, // 23: spaceone.api.identity.v2.WorkspaceGroup.update:output_type -> spaceone.api.identity.v2.WorkspaceGroupInfo
	20, // 24: spaceone.api.identity.v2.WorkspaceGroup.delete:output_type -> google.protobuf.Empty
	10, // 25: spaceone.api.identity.v2.WorkspaceGroup.add_workspaces:output_type -> spaceone.api.identity.v2.WorkspaceGroupInfo
	10, // 26: spaceone.api.identity.v2.WorkspaceGroup.remove_workspaces:output_type -> spaceone.api.identity.v2.WorkspaceGroupInfo
	14, // 27: spaceone.api.identity.v2.WorkspaceGroup.find_users:output_type -> spaceone.api.identity.v2.WorkspaceGroupUsersSummaryInfo
	10, // 28: spaceone.api.identity.v2.WorkspaceGroup.add_users:output_type -> spaceone.api.identity.v2.WorkspaceGroupInfo
	10, // 29: spaceone.api.identity.v2.WorkspaceGroup.remove_users:output_type -> spaceone.api.identity.v2.WorkspaceGroupInfo
	10, // 30: spaceone.api.identity.v2.WorkspaceGroup.get:output_type -> spaceone.api.identity.v2.WorkspaceGroupInfo
	12, // 31: spaceone.api.identity.v2.WorkspaceGroup.list:output_type -> spaceone.api.identity.v2.WorkspaceGroupsInfo
	16, // 32: spaceone.api.identity.v2.WorkspaceGroup.stat:output_type -> google.protobuf.Struct
	22, // [22:33] is the sub-list for method output_type
	11, // [11:22] is the sub-list for method input_type
	11, // [11:11] is the sub-list for extension type_name
	11, // [11:11] is the sub-list for extension extendee
	0,  // [0:11] is the sub-list for field type_name
}

func init() { file_spaceone_api_identity_v2_workspace_group_proto_init() }
func file_spaceone_api_identity_v2_workspace_group_proto_init() {
	if File_spaceone_api_identity_v2_workspace_group_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[0].Exporter = func(v any, i int) any {
			switch v := v.(*CreateWorkspaceGroupRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[1].Exporter = func(v any, i int) any {
			switch v := v.(*UpdateWorkspaceGroupRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[2].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspacesWorkspaceGroupRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[3].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspaceGroupFindRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[4].Exporter = func(v any, i int) any {
			switch v := v.(*AddUsersWorkspaceGroupRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[5].Exporter = func(v any, i int) any {
			switch v := v.(*RemoveUsersWorkspaceGroupRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[6].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspaceGroupRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[7].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspaceGroupUser); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[8].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspaceGroupInfo); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[9].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspaceGroupSearchQuery); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[10].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspaceGroupsInfo); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[11].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspaceGroupUserSummaryInfo); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[12].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspaceGroupUsersSummaryInfo); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_spaceone_api_identity_v2_workspace_group_proto_msgTypes[13].Exporter = func(v any, i int) any {
			switch v := v.(*WorkspaceGroupStatQuery); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_spaceone_api_identity_v2_workspace_group_proto_rawDesc,
			NumEnums:      2,
			NumMessages:   14,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_spaceone_api_identity_v2_workspace_group_proto_goTypes,
		DependencyIndexes: file_spaceone_api_identity_v2_workspace_group_proto_depIdxs,
		EnumInfos:         file_spaceone_api_identity_v2_workspace_group_proto_enumTypes,
		MessageInfos:      file_spaceone_api_identity_v2_workspace_group_proto_msgTypes,
	}.Build()
	File_spaceone_api_identity_v2_workspace_group_proto = out.File
	file_spaceone_api_identity_v2_workspace_group_proto_rawDesc = nil
	file_spaceone_api_identity_v2_workspace_group_proto_goTypes = nil
	file_spaceone_api_identity_v2_workspace_group_proto_depIdxs = nil
}
