// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        v3.6.1
// source: spaceone/api/identity/v2/service_account.proto

package v2

import (
	v2 "github.com/cloudforet-io/api/dist/go/spaceone/api/core/v2"
	empty "github.com/golang/protobuf/ptypes/empty"
	_struct "github.com/golang/protobuf/ptypes/struct"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ServiceAccountInfo_State int32

const (
	ServiceAccountInfo_NONE     ServiceAccountInfo_State = 0
	ServiceAccountInfo_PENDING  ServiceAccountInfo_State = 1
	ServiceAccountInfo_ACTIVE   ServiceAccountInfo_State = 2
	ServiceAccountInfo_INACTIVE ServiceAccountInfo_State = 3
	ServiceAccountInfo_DELETED  ServiceAccountInfo_State = 4
)

// Enum value maps for ServiceAccountInfo_State.
var (
	ServiceAccountInfo_State_name = map[int32]string{
		0: "NONE",
		1: "PENDING",
		2: "ACTIVE",
		3: "INACTIVE",
		4: "DELETED",
	}
	ServiceAccountInfo_State_value = map[string]int32{
		"NONE":     0,
		"PENDING":  1,
		"ACTIVE":   2,
		"INACTIVE": 3,
		"DELETED":  4,
	}
)

func (x ServiceAccountInfo_State) Enum() *ServiceAccountInfo_State {
	p := new(ServiceAccountInfo_State)
	*p = x
	return p
}

func (x ServiceAccountInfo_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ServiceAccountInfo_State) Descriptor() protoreflect.EnumDescriptor {
	return file_spaceone_api_identity_v2_service_account_proto_enumTypes[0].Descriptor()
}

func (ServiceAccountInfo_State) Type() protoreflect.EnumType {
	return &file_spaceone_api_identity_v2_service_account_proto_enumTypes[0]
}

func (x ServiceAccountInfo_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ServiceAccountInfo_State.Descriptor instead.
func (ServiceAccountInfo_State) EnumDescriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{4, 0}
}

type ServiceAccountSearchQuery_State int32

const (
	ServiceAccountSearchQuery_NONE     ServiceAccountSearchQuery_State = 0
	ServiceAccountSearchQuery_PENDING  ServiceAccountSearchQuery_State = 1
	ServiceAccountSearchQuery_ACTIVE   ServiceAccountSearchQuery_State = 2
	ServiceAccountSearchQuery_INACTIVE ServiceAccountSearchQuery_State = 3
	ServiceAccountSearchQuery_DELETED  ServiceAccountSearchQuery_State = 4
)

// Enum value maps for ServiceAccountSearchQuery_State.
var (
	ServiceAccountSearchQuery_State_name = map[int32]string{
		0: "NONE",
		1: "PENDING",
		2: "ACTIVE",
		3: "INACTIVE",
		4: "DELETED",
	}
	ServiceAccountSearchQuery_State_value = map[string]int32{
		"NONE":     0,
		"PENDING":  1,
		"ACTIVE":   2,
		"INACTIVE": 3,
		"DELETED":  4,
	}
)

func (x ServiceAccountSearchQuery_State) Enum() *ServiceAccountSearchQuery_State {
	p := new(ServiceAccountSearchQuery_State)
	*p = x
	return p
}

func (x ServiceAccountSearchQuery_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ServiceAccountSearchQuery_State) Descriptor() protoreflect.EnumDescriptor {
	return file_spaceone_api_identity_v2_service_account_proto_enumTypes[1].Descriptor()
}

func (ServiceAccountSearchQuery_State) Type() protoreflect.EnumType {
	return &file_spaceone_api_identity_v2_service_account_proto_enumTypes[1]
}

func (x ServiceAccountSearchQuery_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ServiceAccountSearchQuery_State.Descriptor instead.
func (ServiceAccountSearchQuery_State) EnumDescriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{5, 0}
}

//	{
//	 "name": "wonny-service-account",
//	 "data": {
//	   "account_id": "a1b2c3d4e5f6"
//	 },
//	 "provider": "aws",
//	 "secret_schema_id": "aws-secret-access-key",
//	 "secret_data": {
//	   "aws_access_key_id": "a1b2c3d4e5f6",
//	   "aws_secret_access_key": "a1b2c3d4e5f6"
//	 },
//	 "tags": {},
//	 "service_account_mgr_id": "wonny@cloudforet.io",
//	 "project_id": "project-a1b2c3d4e5f6"
//	}
type CreateServiceAccountRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Name     string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Data     *_struct.Struct        `protobuf:"bytes,2,opt,name=data,proto3" json:"data,omitempty"`
	Provider string                 `protobuf:"bytes,3,opt,name=provider,proto3" json:"provider,omitempty"`
	// +optional
	SecretSchemaId string `protobuf:"bytes,4,opt,name=secret_schema_id,json=secretSchemaId,proto3" json:"secret_schema_id,omitempty"`
	// +optional
	SecretData *_struct.Struct `protobuf:"bytes,5,opt,name=secret_data,json=secretData,proto3" json:"secret_data,omitempty"`
	// +optional
	Tags *_struct.Struct `protobuf:"bytes,6,opt,name=tags,proto3" json:"tags,omitempty"`
	// +optional
	ProjectId        string `protobuf:"bytes,21,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	TrustedAccountId string `protobuf:"bytes,22,opt,name=trusted_account_id,json=trustedAccountId,proto3" json:"trusted_account_id,omitempty"`
	// service account manager should be in the same domain and this user will be invited automatically in project.
	// +optional
	ServiceAccountMgrId string `protobuf:"bytes,23,opt,name=service_account_mgr_id,json=serviceAccountMgrId,proto3" json:"service_account_mgr_id,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *CreateServiceAccountRequest) Reset() {
	*x = CreateServiceAccountRequest{}
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateServiceAccountRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateServiceAccountRequest) ProtoMessage() {}

func (x *CreateServiceAccountRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateServiceAccountRequest.ProtoReflect.Descriptor instead.
func (*CreateServiceAccountRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{0}
}

func (x *CreateServiceAccountRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateServiceAccountRequest) GetData() *_struct.Struct {
	if x != nil {
		return x.Data
	}
	return nil
}

func (x *CreateServiceAccountRequest) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *CreateServiceAccountRequest) GetSecretSchemaId() string {
	if x != nil {
		return x.SecretSchemaId
	}
	return ""
}

func (x *CreateServiceAccountRequest) GetSecretData() *_struct.Struct {
	if x != nil {
		return x.SecretData
	}
	return nil
}

func (x *CreateServiceAccountRequest) GetTags() *_struct.Struct {
	if x != nil {
		return x.Tags
	}
	return nil
}

func (x *CreateServiceAccountRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *CreateServiceAccountRequest) GetTrustedAccountId() string {
	if x != nil {
		return x.TrustedAccountId
	}
	return ""
}

func (x *CreateServiceAccountRequest) GetServiceAccountMgrId() string {
	if x != nil {
		return x.ServiceAccountMgrId
	}
	return ""
}

//	{
//	 "name": "cloudforet-service-account 2",
//	 "data": {
//	   "account_id": "g7h8i9j1k2l3"
//	 },
//	 "tags": {},
//	 "service_account_mgr_id": "wonny@cloudforet.io"
//	 "service_account_id": "sa-a1b2c3d4e5f6",
//	 "project_id": "project-a1b2c3d4e5f6"
//	}
type UpdateServiceAccountRequest struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	ServiceAccountId string                 `protobuf:"bytes,1,opt,name=service_account_id,json=serviceAccountId,proto3" json:"service_account_id,omitempty"`
	// +optional
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// +optional
	Data *_struct.Struct `protobuf:"bytes,3,opt,name=data,proto3" json:"data,omitempty"`
	// +optional
	Tags *_struct.Struct `protobuf:"bytes,4,opt,name=tags,proto3" json:"tags,omitempty"`
	// +optional
	ProjectId string `protobuf:"bytes,21,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// +optional
	ServiceAccountMgrId string `protobuf:"bytes,22,opt,name=service_account_mgr_id,json=serviceAccountMgrId,proto3" json:"service_account_mgr_id,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *UpdateServiceAccountRequest) Reset() {
	*x = UpdateServiceAccountRequest{}
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateServiceAccountRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateServiceAccountRequest) ProtoMessage() {}

func (x *UpdateServiceAccountRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateServiceAccountRequest.ProtoReflect.Descriptor instead.
func (*UpdateServiceAccountRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{1}
}

func (x *UpdateServiceAccountRequest) GetServiceAccountId() string {
	if x != nil {
		return x.ServiceAccountId
	}
	return ""
}

func (x *UpdateServiceAccountRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpdateServiceAccountRequest) GetData() *_struct.Struct {
	if x != nil {
		return x.Data
	}
	return nil
}

func (x *UpdateServiceAccountRequest) GetTags() *_struct.Struct {
	if x != nil {
		return x.Tags
	}
	return nil
}

func (x *UpdateServiceAccountRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *UpdateServiceAccountRequest) GetServiceAccountMgrId() string {
	if x != nil {
		return x.ServiceAccountMgrId
	}
	return ""
}

//	{
//	 "service_account_id": "sa-a1b2c3d4e5f6",
//	 "secret_schema_id": "aws-secret-access-key",
//	 "secret_data": {
//	   "aws_access_key_id": "a1b2c3d4e5f6",
//	   "aws_secret_access_key": "a1b2c3d4e5f6"
//	 }
//	}
type UpdateServiceAccountSecretRequest struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	ServiceAccountId string                 `protobuf:"bytes,1,opt,name=service_account_id,json=serviceAccountId,proto3" json:"service_account_id,omitempty"`
	SecretSchemaId   string                 `protobuf:"bytes,2,opt,name=secret_schema_id,json=secretSchemaId,proto3" json:"secret_schema_id,omitempty"`
	SecretData       *_struct.Struct        `protobuf:"bytes,3,opt,name=secret_data,json=secretData,proto3" json:"secret_data,omitempty"`
	// +optional
	TrustedAccountId string `protobuf:"bytes,21,opt,name=trusted_account_id,json=trustedAccountId,proto3" json:"trusted_account_id,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *UpdateServiceAccountSecretRequest) Reset() {
	*x = UpdateServiceAccountSecretRequest{}
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateServiceAccountSecretRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateServiceAccountSecretRequest) ProtoMessage() {}

func (x *UpdateServiceAccountSecretRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateServiceAccountSecretRequest.ProtoReflect.Descriptor instead.
func (*UpdateServiceAccountSecretRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{2}
}

func (x *UpdateServiceAccountSecretRequest) GetServiceAccountId() string {
	if x != nil {
		return x.ServiceAccountId
	}
	return ""
}

func (x *UpdateServiceAccountSecretRequest) GetSecretSchemaId() string {
	if x != nil {
		return x.SecretSchemaId
	}
	return ""
}

func (x *UpdateServiceAccountSecretRequest) GetSecretData() *_struct.Struct {
	if x != nil {
		return x.SecretData
	}
	return nil
}

func (x *UpdateServiceAccountSecretRequest) GetTrustedAccountId() string {
	if x != nil {
		return x.TrustedAccountId
	}
	return ""
}

//	{
//	 "service_account_id": "sa-a1b2c3d4e5f6"
//	}
type ServiceAccountRequest struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	ServiceAccountId string                 `protobuf:"bytes,1,opt,name=service_account_id,json=serviceAccountId,proto3" json:"service_account_id,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *ServiceAccountRequest) Reset() {
	*x = ServiceAccountRequest{}
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ServiceAccountRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ServiceAccountRequest) ProtoMessage() {}

func (x *ServiceAccountRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ServiceAccountRequest.ProtoReflect.Descriptor instead.
func (*ServiceAccountRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{3}
}

func (x *ServiceAccountRequest) GetServiceAccountId() string {
	if x != nil {
		return x.ServiceAccountId
	}
	return ""
}

//	{
//	 "state": "ACTIVE",
//	 "data": {
//	   "account_id": "a1b2c3d4e5f6"
//	 },
//	 "name": "test-service-account",
//	 "provider": "aws",
//	 "secret_id": "secret-a1b2c3d4e5f6",
//	 "secret_schema_id": "aws-secret-access-key",
//	 "service_account_mgr_id": "wonny@cloudforet.io",
//	 "service_account_id": "sa-a1b2c3d4e5f6",
//	 "project_id": "project-a1b2c3d4e5f6",
//	 "workspace_id": "workspace-a1b2c3d4e5f6"
//	 "domain_id": "domain-a1b2c3d4e5f6",
//	 "created_at": "2024-11-18T05:39:08.732Z"
//	}
type ServiceAccountInfo struct {
	state               protoimpl.MessageState   `protogen:"open.v1"`
	ServiceAccountId    string                   `protobuf:"bytes,1,opt,name=service_account_id,json=serviceAccountId,proto3" json:"service_account_id,omitempty"`
	Name                string                   `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	State               ServiceAccountInfo_State `protobuf:"varint,3,opt,name=state,proto3,enum=spaceone.api.identity.v2.ServiceAccountInfo_State" json:"state,omitempty"`
	Data                *_struct.Struct          `protobuf:"bytes,4,opt,name=data,proto3" json:"data,omitempty"`
	Provider            string                   `protobuf:"bytes,5,opt,name=provider,proto3" json:"provider,omitempty"`
	Tags                *_struct.Struct          `protobuf:"bytes,6,opt,name=tags,proto3" json:"tags,omitempty"`
	ReferenceId         string                   `protobuf:"bytes,7,opt,name=reference_id,json=referenceId,proto3" json:"reference_id,omitempty"`
	IsManaged           bool                     `protobuf:"varint,8,opt,name=is_managed,json=isManaged,proto3" json:"is_managed,omitempty"`
	DomainId            string                   `protobuf:"bytes,21,opt,name=domain_id,json=domainId,proto3" json:"domain_id,omitempty"`
	WorkspaceId         string                   `protobuf:"bytes,22,opt,name=workspace_id,json=workspaceId,proto3" json:"workspace_id,omitempty"`
	ProjectId           string                   `protobuf:"bytes,23,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	TrustedAccountId    string                   `protobuf:"bytes,24,opt,name=trusted_account_id,json=trustedAccountId,proto3" json:"trusted_account_id,omitempty"`
	SecretSchemaId      string                   `protobuf:"bytes,25,opt,name=secret_schema_id,json=secretSchemaId,proto3" json:"secret_schema_id,omitempty"`
	SecretId            string                   `protobuf:"bytes,26,opt,name=secret_id,json=secretId,proto3" json:"secret_id,omitempty"`
	ServiceAccountMgrId string                   `protobuf:"bytes,27,opt,name=service_account_mgr_id,json=serviceAccountMgrId,proto3" json:"service_account_mgr_id,omitempty"`
	CreatedAt           string                   `protobuf:"bytes,31,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	LastSyncedAt        string                   `protobuf:"bytes,32,opt,name=last_synced_at,json=lastSyncedAt,proto3" json:"last_synced_at,omitempty"`
	DeletedAt           string                   `protobuf:"bytes,33,opt,name=deleted_at,json=deletedAt,proto3" json:"deleted_at,omitempty"`
	InactivatedAt       string                   `protobuf:"bytes,34,opt,name=inactivated_at,json=inactivatedAt,proto3" json:"inactivated_at,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *ServiceAccountInfo) Reset() {
	*x = ServiceAccountInfo{}
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ServiceAccountInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ServiceAccountInfo) ProtoMessage() {}

func (x *ServiceAccountInfo) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ServiceAccountInfo.ProtoReflect.Descriptor instead.
func (*ServiceAccountInfo) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{4}
}

func (x *ServiceAccountInfo) GetServiceAccountId() string {
	if x != nil {
		return x.ServiceAccountId
	}
	return ""
}

func (x *ServiceAccountInfo) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ServiceAccountInfo) GetState() ServiceAccountInfo_State {
	if x != nil {
		return x.State
	}
	return ServiceAccountInfo_NONE
}

func (x *ServiceAccountInfo) GetData() *_struct.Struct {
	if x != nil {
		return x.Data
	}
	return nil
}

func (x *ServiceAccountInfo) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *ServiceAccountInfo) GetTags() *_struct.Struct {
	if x != nil {
		return x.Tags
	}
	return nil
}

func (x *ServiceAccountInfo) GetReferenceId() string {
	if x != nil {
		return x.ReferenceId
	}
	return ""
}

func (x *ServiceAccountInfo) GetIsManaged() bool {
	if x != nil {
		return x.IsManaged
	}
	return false
}

func (x *ServiceAccountInfo) GetDomainId() string {
	if x != nil {
		return x.DomainId
	}
	return ""
}

func (x *ServiceAccountInfo) GetWorkspaceId() string {
	if x != nil {
		return x.WorkspaceId
	}
	return ""
}

func (x *ServiceAccountInfo) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *ServiceAccountInfo) GetTrustedAccountId() string {
	if x != nil {
		return x.TrustedAccountId
	}
	return ""
}

func (x *ServiceAccountInfo) GetSecretSchemaId() string {
	if x != nil {
		return x.SecretSchemaId
	}
	return ""
}

func (x *ServiceAccountInfo) GetSecretId() string {
	if x != nil {
		return x.SecretId
	}
	return ""
}

func (x *ServiceAccountInfo) GetServiceAccountMgrId() string {
	if x != nil {
		return x.ServiceAccountMgrId
	}
	return ""
}

func (x *ServiceAccountInfo) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

func (x *ServiceAccountInfo) GetLastSyncedAt() string {
	if x != nil {
		return x.LastSyncedAt
	}
	return ""
}

func (x *ServiceAccountInfo) GetDeletedAt() string {
	if x != nil {
		return x.DeletedAt
	}
	return ""
}

func (x *ServiceAccountInfo) GetInactivatedAt() string {
	if x != nil {
		return x.InactivatedAt
	}
	return ""
}

//	{
//	 "provider": "aws",
//	 "query": {
//	   "page": {
//	     "start": 1,
//	     "limit": 10
//	   },
//	   "sort": [
//	     {
//	       "key": "created_at",
//	       "desc": true
//	     }
//	   ]
//	 }
//	}
type ServiceAccountSearchQuery struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// +optional
	Query *v2.Query `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	// +optional
	ServiceAccountId string `protobuf:"bytes,2,opt,name=service_account_id,json=serviceAccountId,proto3" json:"service_account_id,omitempty"`
	// +optional
	Name string `protobuf:"bytes,3,opt,name=name,proto3" json:"name,omitempty"`
	// +optional
	State string `protobuf:"bytes,4,opt,name=state,proto3" json:"state,omitempty"`
	// +optional
	Provider string `protobuf:"bytes,5,opt,name=provider,proto3" json:"provider,omitempty"`
	// +optional
	HasSecret bool `protobuf:"varint,6,opt,name=has_secret,json=hasSecret,proto3" json:"has_secret,omitempty"`
	// +optional
	WorkspaceId string `protobuf:"bytes,21,opt,name=workspace_id,json=workspaceId,proto3" json:"workspace_id,omitempty"`
	// +optional
	ProjectId string `protobuf:"bytes,23,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// +optional
	TrustedAccountId string `protobuf:"bytes,24,opt,name=trusted_account_id,json=trustedAccountId,proto3" json:"trusted_account_id,omitempty"`
	// +optional
	SecretSchemaId string `protobuf:"bytes,25,opt,name=secret_schema_id,json=secretSchemaId,proto3" json:"secret_schema_id,omitempty"`
	// +optional
	SecretId      string `protobuf:"bytes,26,opt,name=secret_id,json=secretId,proto3" json:"secret_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ServiceAccountSearchQuery) Reset() {
	*x = ServiceAccountSearchQuery{}
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ServiceAccountSearchQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ServiceAccountSearchQuery) ProtoMessage() {}

func (x *ServiceAccountSearchQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ServiceAccountSearchQuery.ProtoReflect.Descriptor instead.
func (*ServiceAccountSearchQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{5}
}

func (x *ServiceAccountSearchQuery) GetQuery() *v2.Query {
	if x != nil {
		return x.Query
	}
	return nil
}

func (x *ServiceAccountSearchQuery) GetServiceAccountId() string {
	if x != nil {
		return x.ServiceAccountId
	}
	return ""
}

func (x *ServiceAccountSearchQuery) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ServiceAccountSearchQuery) GetState() string {
	if x != nil {
		return x.State
	}
	return ""
}

func (x *ServiceAccountSearchQuery) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *ServiceAccountSearchQuery) GetHasSecret() bool {
	if x != nil {
		return x.HasSecret
	}
	return false
}

func (x *ServiceAccountSearchQuery) GetWorkspaceId() string {
	if x != nil {
		return x.WorkspaceId
	}
	return ""
}

func (x *ServiceAccountSearchQuery) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *ServiceAccountSearchQuery) GetTrustedAccountId() string {
	if x != nil {
		return x.TrustedAccountId
	}
	return ""
}

func (x *ServiceAccountSearchQuery) GetSecretSchemaId() string {
	if x != nil {
		return x.SecretSchemaId
	}
	return ""
}

func (x *ServiceAccountSearchQuery) GetSecretId() string {
	if x != nil {
		return x.SecretId
	}
	return ""
}

//	{
//	 "results": [
//	   {
//	     "name": "woony-service-account",
//	     "data": {
//	       "account_id": "a1b2c3d4e5f6"
//	     },
//	     "provider": "aws",
//	     "state": "ACTIVE",
//	     "tags": {},
//	     "service_account_mgr_id": "wonny@cloudforet.io",
//	     "service_account_id": "sa-a1b2c3d4e5f6",
//	     "project_id": "project-a1b2c3d4e5f6",
//	     "workspace_id": "workspace-a1b2c3d4e5f6"
//	     "domain_id": "domain-a1b2c3d4e5f6",
//	     "created_at": "2024-11-18T06:03:09.191Z",
//	   },
//	   {
//	     "name": "cloudforet-service-account",
//	     "state": "ACTIVE",
//	     "data": {
//	       "account_id": "g7h8i9j1k2l3"
//	     },
//	     "provider": "aws",
//	     "tags": {},
//	     "secret_id": "secret-g7h8i9j1k2l3",
//	     "secret_schema_id": "aws-secret-access-key",
//	     "service_account_mgr_id": "wonny@cloudforet.io",
//	     "service_account_id": "sa-g7h8i9j1k2l3",
//	     "project_id": "project-g7h8i9j1k2l3",
//	     "workspace_id": "workspace-g7h8i9j1k2l3"
//	     "domain_id": "domain-a1b2c3d4e5f6",
//	     "created_at": "2024-11-18T05:48:49.783Z",
//	   }
//	 ],
//	 "total_count": 2
//	}
type ServiceAccountsInfo struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Results       []*ServiceAccountInfo  `protobuf:"bytes,1,rep,name=results,proto3" json:"results,omitempty"`
	TotalCount    int32                  `protobuf:"varint,2,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ServiceAccountsInfo) Reset() {
	*x = ServiceAccountsInfo{}
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ServiceAccountsInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ServiceAccountsInfo) ProtoMessage() {}

func (x *ServiceAccountsInfo) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ServiceAccountsInfo.ProtoReflect.Descriptor instead.
func (*ServiceAccountsInfo) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{6}
}

func (x *ServiceAccountsInfo) GetResults() []*ServiceAccountInfo {
	if x != nil {
		return x.Results
	}
	return nil
}

func (x *ServiceAccountsInfo) GetTotalCount() int32 {
	if x != nil {
		return x.TotalCount
	}
	return 0
}

type ServiceAccountAnalyzeQuery struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Query         *v2.AnalyzeQuery       `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ServiceAccountAnalyzeQuery) Reset() {
	*x = ServiceAccountAnalyzeQuery{}
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ServiceAccountAnalyzeQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ServiceAccountAnalyzeQuery) ProtoMessage() {}

func (x *ServiceAccountAnalyzeQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ServiceAccountAnalyzeQuery.ProtoReflect.Descriptor instead.
func (*ServiceAccountAnalyzeQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{7}
}

func (x *ServiceAccountAnalyzeQuery) GetQuery() *v2.AnalyzeQuery {
	if x != nil {
		return x.Query
	}
	return nil
}

type ServiceAccountStatQuery struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Query         *v2.StatisticsQuery    `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ServiceAccountStatQuery) Reset() {
	*x = ServiceAccountStatQuery{}
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ServiceAccountStatQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ServiceAccountStatQuery) ProtoMessage() {}

func (x *ServiceAccountStatQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_identity_v2_service_account_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ServiceAccountStatQuery.ProtoReflect.Descriptor instead.
func (*ServiceAccountStatQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP(), []int{8}
}

func (x *ServiceAccountStatQuery) GetQuery() *v2.StatisticsQuery {
	if x != nil {
		return x.Query
	}
	return nil
}

var File_spaceone_api_identity_v2_service_account_proto protoreflect.FileDescriptor

const file_spaceone_api_identity_v2_service_account_proto_rawDesc = "" +
	"\n" +
	".spaceone/api/identity/v2/service_account.proto\x12\x18spaceone.api.identity.v2\x1a\x1bgoogle/protobuf/empty.proto\x1a\x1cgoogle/protobuf/struct.proto\x1a\x1cgoogle/api/annotations.proto\x1a spaceone/api/core/v2/query.proto\x1a\"spaceone/api/identity/v2/app.proto\"\x8d\x03\n" +
	"\x1bCreateServiceAccountRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12+\n" +
	"\x04data\x18\x02 \x01(\v2\x17.google.protobuf.StructR\x04data\x12\x1a\n" +
	"\bprovider\x18\x03 \x01(\tR\bprovider\x12(\n" +
	"\x10secret_schema_id\x18\x04 \x01(\tR\x0esecretSchemaId\x128\n" +
	"\vsecret_data\x18\x05 \x01(\v2\x17.google.protobuf.StructR\n" +
	"secretData\x12+\n" +
	"\x04tags\x18\x06 \x01(\v2\x17.google.protobuf.StructR\x04tags\x12\x1d\n" +
	"\n" +
	"project_id\x18\x15 \x01(\tR\tprojectId\x12,\n" +
	"\x12trusted_account_id\x18\x16 \x01(\tR\x10trustedAccountId\x123\n" +
	"\x16service_account_mgr_id\x18\x17 \x01(\tR\x13serviceAccountMgrId\"\x8d\x02\n" +
	"\x1bUpdateServiceAccountRequest\x12,\n" +
	"\x12service_account_id\x18\x01 \x01(\tR\x10serviceAccountId\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12+\n" +
	"\x04data\x18\x03 \x01(\v2\x17.google.protobuf.StructR\x04data\x12+\n" +
	"\x04tags\x18\x04 \x01(\v2\x17.google.protobuf.StructR\x04tags\x12\x1d\n" +
	"\n" +
	"project_id\x18\x15 \x01(\tR\tprojectId\x123\n" +
	"\x16service_account_mgr_id\x18\x16 \x01(\tR\x13serviceAccountMgrId\"\xe3\x01\n" +
	"!UpdateServiceAccountSecretRequest\x12,\n" +
	"\x12service_account_id\x18\x01 \x01(\tR\x10serviceAccountId\x12(\n" +
	"\x10secret_schema_id\x18\x02 \x01(\tR\x0esecretSchemaId\x128\n" +
	"\vsecret_data\x18\x03 \x01(\v2\x17.google.protobuf.StructR\n" +
	"secretData\x12,\n" +
	"\x12trusted_account_id\x18\x15 \x01(\tR\x10trustedAccountId\"E\n" +
	"\x15ServiceAccountRequest\x12,\n" +
	"\x12service_account_id\x18\x01 \x01(\tR\x10serviceAccountId\"\xb3\x06\n" +
	"\x12ServiceAccountInfo\x12,\n" +
	"\x12service_account_id\x18\x01 \x01(\tR\x10serviceAccountId\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12H\n" +
	"\x05state\x18\x03 \x01(\x0e22.spaceone.api.identity.v2.ServiceAccountInfo.StateR\x05state\x12+\n" +
	"\x04data\x18\x04 \x01(\v2\x17.google.protobuf.StructR\x04data\x12\x1a\n" +
	"\bprovider\x18\x05 \x01(\tR\bprovider\x12+\n" +
	"\x04tags\x18\x06 \x01(\v2\x17.google.protobuf.StructR\x04tags\x12!\n" +
	"\freference_id\x18\a \x01(\tR\vreferenceId\x12\x1d\n" +
	"\n" +
	"is_managed\x18\b \x01(\bR\tisManaged\x12\x1b\n" +
	"\tdomain_id\x18\x15 \x01(\tR\bdomainId\x12!\n" +
	"\fworkspace_id\x18\x16 \x01(\tR\vworkspaceId\x12\x1d\n" +
	"\n" +
	"project_id\x18\x17 \x01(\tR\tprojectId\x12,\n" +
	"\x12trusted_account_id\x18\x18 \x01(\tR\x10trustedAccountId\x12(\n" +
	"\x10secret_schema_id\x18\x19 \x01(\tR\x0esecretSchemaId\x12\x1b\n" +
	"\tsecret_id\x18\x1a \x01(\tR\bsecretId\x123\n" +
	"\x16service_account_mgr_id\x18\x1b \x01(\tR\x13serviceAccountMgrId\x12\x1d\n" +
	"\n" +
	"created_at\x18\x1f \x01(\tR\tcreatedAt\x12$\n" +
	"\x0elast_synced_at\x18  \x01(\tR\flastSyncedAt\x12\x1d\n" +
	"\n" +
	"deleted_at\x18! \x01(\tR\tdeletedAt\x12%\n" +
	"\x0einactivated_at\x18\" \x01(\tR\rinactivatedAt\"E\n" +
	"\x05State\x12\b\n" +
	"\x04NONE\x10\x00\x12\v\n" +
	"\aPENDING\x10\x01\x12\n" +
	"\n" +
	"\x06ACTIVE\x10\x02\x12\f\n" +
	"\bINACTIVE\x10\x03\x12\v\n" +
	"\aDELETED\x10\x04\"\xdf\x03\n" +
	"\x19ServiceAccountSearchQuery\x121\n" +
	"\x05query\x18\x01 \x01(\v2\x1b.spaceone.api.core.v2.QueryR\x05query\x12,\n" +
	"\x12service_account_id\x18\x02 \x01(\tR\x10serviceAccountId\x12\x12\n" +
	"\x04name\x18\x03 \x01(\tR\x04name\x12\x14\n" +
	"\x05state\x18\x04 \x01(\tR\x05state\x12\x1a\n" +
	"\bprovider\x18\x05 \x01(\tR\bprovider\x12\x1d\n" +
	"\n" +
	"has_secret\x18\x06 \x01(\bR\thasSecret\x12!\n" +
	"\fworkspace_id\x18\x15 \x01(\tR\vworkspaceId\x12\x1d\n" +
	"\n" +
	"project_id\x18\x17 \x01(\tR\tprojectId\x12,\n" +
	"\x12trusted_account_id\x18\x18 \x01(\tR\x10trustedAccountId\x12(\n" +
	"\x10secret_schema_id\x18\x19 \x01(\tR\x0esecretSchemaId\x12\x1b\n" +
	"\tsecret_id\x18\x1a \x01(\tR\bsecretId\"E\n" +
	"\x05State\x12\b\n" +
	"\x04NONE\x10\x00\x12\v\n" +
	"\aPENDING\x10\x01\x12\n" +
	"\n" +
	"\x06ACTIVE\x10\x02\x12\f\n" +
	"\bINACTIVE\x10\x03\x12\v\n" +
	"\aDELETED\x10\x04\"~\n" +
	"\x13ServiceAccountsInfo\x12F\n" +
	"\aresults\x18\x01 \x03(\v2,.spaceone.api.identity.v2.ServiceAccountInfoR\aresults\x12\x1f\n" +
	"\vtotal_count\x18\x02 \x01(\x05R\n" +
	"totalCount\"V\n" +
	"\x1aServiceAccountAnalyzeQuery\x128\n" +
	"\x05query\x18\x01 \x01(\v2\".spaceone.api.core.v2.AnalyzeQueryR\x05query\"V\n" +
	"\x17ServiceAccountStatQuery\x12;\n" +
	"\x05query\x18\x01 \x01(\v2%.spaceone.api.core.v2.StatisticsQueryR\x05query2\x82\v\n" +
	"\x0eServiceAccount\x12\x9d\x01\n" +
	"\x06create\x125.spaceone.api.identity.v2.CreateServiceAccountRequest\x1a,.spaceone.api.identity.v2.ServiceAccountInfo\".\x82\xd3\xe4\x93\x02(:\x01*\"#/identity/v2/service-account/create\x12\x9d\x01\n" +
	"\x06update\x125.spaceone.api.identity.v2.UpdateServiceAccountRequest\x1a,.spaceone.api.identity.v2.ServiceAccountInfo\".\x82\xd3\xe4\x93\x02(:\x01*\"#/identity/v2/service-account/update\x12\xbb\x01\n" +
	"\x12update_secret_data\x12;.spaceone.api.identity.v2.UpdateServiceAccountSecretRequest\x1a,.spaceone.api.identity.v2.ServiceAccountInfo\":\x82\xd3\xe4\x93\x024:\x01*\"//identity/v2/service-account/update-secret-data\x12\xaf\x01\n" +
	"\x12delete_secret_data\x12/.spaceone.api.identity.v2.ServiceAccountRequest\x1a,.spaceone.api.identity.v2.ServiceAccountInfo\":\x82\xd3\xe4\x93\x024:\x01*\"//identity/v2/service-account/delete-secret-data\x12\x81\x01\n" +
	"\x06delete\x12/.spaceone.api.identity.v2.ServiceAccountRequest\x1a\x16.google.protobuf.Empty\".\x82\xd3\xe4\x93\x02(:\x01*\"#/identity/v2/service-account/delete\x12\x91\x01\n" +
	"\x03get\x12/.spaceone.api.identity.v2.ServiceAccountRequest\x1a,.spaceone.api.identity.v2.ServiceAccountInfo\"+\x82\xd3\xe4\x93\x02%:\x01*\" /identity/v2/service-account/get\x12\x98\x01\n" +
	"\x04list\x123.spaceone.api.identity.v2.ServiceAccountSearchQuery\x1a-.spaceone.api.identity.v2.ServiceAccountsInfo\",\x82\xd3\xe4\x93\x02&:\x01*\"!/identity/v2/service-account/list\x12\x89\x01\n" +
	"\aanalyze\x124.spaceone.api.identity.v2.ServiceAccountAnalyzeQuery\x1a\x17.google.protobuf.Struct\"/\x82\xd3\xe4\x93\x02):\x01*\"$/identity/v2/service-account/analyze\x12\x80\x01\n" +
	"\x04stat\x121.spaceone.api.identity.v2.ServiceAccountStatQuery\x1a\x17.google.protobuf.Struct\",\x82\xd3\xe4\x93\x02&:\x01*\"!/identity/v2/service-account/statB?Z=github.com/cloudforet-io/api/dist/go/spaceone/api/identity/v2b\x06proto3"

var (
	file_spaceone_api_identity_v2_service_account_proto_rawDescOnce sync.Once
	file_spaceone_api_identity_v2_service_account_proto_rawDescData []byte
)

func file_spaceone_api_identity_v2_service_account_proto_rawDescGZIP() []byte {
	file_spaceone_api_identity_v2_service_account_proto_rawDescOnce.Do(func() {
		file_spaceone_api_identity_v2_service_account_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_spaceone_api_identity_v2_service_account_proto_rawDesc), len(file_spaceone_api_identity_v2_service_account_proto_rawDesc)))
	})
	return file_spaceone_api_identity_v2_service_account_proto_rawDescData
}

var file_spaceone_api_identity_v2_service_account_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_spaceone_api_identity_v2_service_account_proto_msgTypes = make([]protoimpl.MessageInfo, 9)
var file_spaceone_api_identity_v2_service_account_proto_goTypes = []any{
	(ServiceAccountInfo_State)(0),             // 0: spaceone.api.identity.v2.ServiceAccountInfo.State
	(ServiceAccountSearchQuery_State)(0),      // 1: spaceone.api.identity.v2.ServiceAccountSearchQuery.State
	(*CreateServiceAccountRequest)(nil),       // 2: spaceone.api.identity.v2.CreateServiceAccountRequest
	(*UpdateServiceAccountRequest)(nil),       // 3: spaceone.api.identity.v2.UpdateServiceAccountRequest
	(*UpdateServiceAccountSecretRequest)(nil), // 4: spaceone.api.identity.v2.UpdateServiceAccountSecretRequest
	(*ServiceAccountRequest)(nil),             // 5: spaceone.api.identity.v2.ServiceAccountRequest
	(*ServiceAccountInfo)(nil),                // 6: spaceone.api.identity.v2.ServiceAccountInfo
	(*ServiceAccountSearchQuery)(nil),         // 7: spaceone.api.identity.v2.ServiceAccountSearchQuery
	(*ServiceAccountsInfo)(nil),               // 8: spaceone.api.identity.v2.ServiceAccountsInfo
	(*ServiceAccountAnalyzeQuery)(nil),        // 9: spaceone.api.identity.v2.ServiceAccountAnalyzeQuery
	(*ServiceAccountStatQuery)(nil),           // 10: spaceone.api.identity.v2.ServiceAccountStatQuery
	(*_struct.Struct)(nil),                    // 11: google.protobuf.Struct
	(*v2.Query)(nil),                          // 12: spaceone.api.core.v2.Query
	(*v2.AnalyzeQuery)(nil),                   // 13: spaceone.api.core.v2.AnalyzeQuery
	(*v2.StatisticsQuery)(nil),                // 14: spaceone.api.core.v2.StatisticsQuery
	(*empty.Empty)(nil),                       // 15: google.protobuf.Empty
}
var file_spaceone_api_identity_v2_service_account_proto_depIdxs = []int32{
	11, // 0: spaceone.api.identity.v2.CreateServiceAccountRequest.data:type_name -> google.protobuf.Struct
	11, // 1: spaceone.api.identity.v2.CreateServiceAccountRequest.secret_data:type_name -> google.protobuf.Struct
	11, // 2: spaceone.api.identity.v2.CreateServiceAccountRequest.tags:type_name -> google.protobuf.Struct
	11, // 3: spaceone.api.identity.v2.UpdateServiceAccountRequest.data:type_name -> google.protobuf.Struct
	11, // 4: spaceone.api.identity.v2.UpdateServiceAccountRequest.tags:type_name -> google.protobuf.Struct
	11, // 5: spaceone.api.identity.v2.UpdateServiceAccountSecretRequest.secret_data:type_name -> google.protobuf.Struct
	0,  // 6: spaceone.api.identity.v2.ServiceAccountInfo.state:type_name -> spaceone.api.identity.v2.ServiceAccountInfo.State
	11, // 7: spaceone.api.identity.v2.ServiceAccountInfo.data:type_name -> google.protobuf.Struct
	11, // 8: spaceone.api.identity.v2.ServiceAccountInfo.tags:type_name -> google.protobuf.Struct
	12, // 9: spaceone.api.identity.v2.ServiceAccountSearchQuery.query:type_name -> spaceone.api.core.v2.Query
	6,  // 10: spaceone.api.identity.v2.ServiceAccountsInfo.results:type_name -> spaceone.api.identity.v2.ServiceAccountInfo
	13, // 11: spaceone.api.identity.v2.ServiceAccountAnalyzeQuery.query:type_name -> spaceone.api.core.v2.AnalyzeQuery
	14, // 12: spaceone.api.identity.v2.ServiceAccountStatQuery.query:type_name -> spaceone.api.core.v2.StatisticsQuery
	2,  // 13: spaceone.api.identity.v2.ServiceAccount.create:input_type -> spaceone.api.identity.v2.CreateServiceAccountRequest
	3,  // 14: spaceone.api.identity.v2.ServiceAccount.update:input_type -> spaceone.api.identity.v2.UpdateServiceAccountRequest
	4,  // 15: spaceone.api.identity.v2.ServiceAccount.update_secret_data:input_type -> spaceone.api.identity.v2.UpdateServiceAccountSecretRequest
	5,  // 16: spaceone.api.identity.v2.ServiceAccount.delete_secret_data:input_type -> spaceone.api.identity.v2.ServiceAccountRequest
	5,  // 17: spaceone.api.identity.v2.ServiceAccount.delete:input_type -> spaceone.api.identity.v2.ServiceAccountRequest
	5,  // 18: spaceone.api.identity.v2.ServiceAccount.get:input_type -> spaceone.api.identity.v2.ServiceAccountRequest
	7,  // 19: spaceone.api.identity.v2.ServiceAccount.list:input_type -> spaceone.api.identity.v2.ServiceAccountSearchQuery
	9,  // 20: spaceone.api.identity.v2.ServiceAccount.analyze:input_type -> spaceone.api.identity.v2.ServiceAccountAnalyzeQuery
	10, // 21: spaceone.api.identity.v2.ServiceAccount.stat:input_type -> spaceone.api.identity.v2.ServiceAccountStatQuery
	6,  // 22: spaceone.api.identity.v2.ServiceAccount.create:output_type -> spaceone.api.identity.v2.ServiceAccountInfo
	6,  // 23: spaceone.api.identity.v2.ServiceAccount.update:output_type -> spaceone.api.identity.v2.ServiceAccountInfo
	6,  // 24: spaceone.api.identity.v2.ServiceAccount.update_secret_data:output_type -> spaceone.api.identity.v2.ServiceAccountInfo
	6,  // 25: spaceone.api.identity.v2.ServiceAccount.delete_secret_data:output_type -> spaceone.api.identity.v2.ServiceAccountInfo
	15, // 26: spaceone.api.identity.v2.ServiceAccount.delete:output_type -> google.protobuf.Empty
	6,  // 27: spaceone.api.identity.v2.ServiceAccount.get:output_type -> spaceone.api.identity.v2.ServiceAccountInfo
	8,  // 28: spaceone.api.identity.v2.ServiceAccount.list:output_type -> spaceone.api.identity.v2.ServiceAccountsInfo
	11, // 29: spaceone.api.identity.v2.ServiceAccount.analyze:output_type -> google.protobuf.Struct
	11, // 30: spaceone.api.identity.v2.ServiceAccount.stat:output_type -> google.protobuf.Struct
	22, // [22:31] is the sub-list for method output_type
	13, // [13:22] is the sub-list for method input_type
	13, // [13:13] is the sub-list for extension type_name
	13, // [13:13] is the sub-list for extension extendee
	0,  // [0:13] is the sub-list for field type_name
}

func init() { file_spaceone_api_identity_v2_service_account_proto_init() }
func file_spaceone_api_identity_v2_service_account_proto_init() {
	if File_spaceone_api_identity_v2_service_account_proto != nil {
		return
	}
	file_spaceone_api_identity_v2_app_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_spaceone_api_identity_v2_service_account_proto_rawDesc), len(file_spaceone_api_identity_v2_service_account_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   9,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_spaceone_api_identity_v2_service_account_proto_goTypes,
		DependencyIndexes: file_spaceone_api_identity_v2_service_account_proto_depIdxs,
		EnumInfos:         file_spaceone_api_identity_v2_service_account_proto_enumTypes,
		MessageInfos:      file_spaceone_api_identity_v2_service_account_proto_msgTypes,
	}.Build()
	File_spaceone_api_identity_v2_service_account_proto = out.File
	file_spaceone_api_identity_v2_service_account_proto_goTypes = nil
	file_spaceone_api_identity_v2_service_account_proto_depIdxs = nil
}
