// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        v3.6.1
// source: spaceone/api/core/v2/query.proto

package v2

import (
	_struct "github.com/golang/protobuf/ptypes/struct"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type TimeSeriesAnalyzeQuery_Granularity int32

const (
	TimeSeriesAnalyzeQuery_GRANULARITY_NONE TimeSeriesAnalyzeQuery_Granularity = 0
	TimeSeriesAnalyzeQuery_DAILY            TimeSeriesAnalyzeQuery_Granularity = 1
	TimeSeriesAnalyzeQuery_MONTHLY          TimeSeriesAnalyzeQuery_Granularity = 2
	TimeSeriesAnalyzeQuery_YEARLY           TimeSeriesAnalyzeQuery_Granularity = 3
)

// Enum value maps for TimeSeriesAnalyzeQuery_Granularity.
var (
	TimeSeriesAnalyzeQuery_Granularity_name = map[int32]string{
		0: "GRANULARITY_NONE",
		1: "DAILY",
		2: "MONTHLY",
		3: "YEARLY",
	}
	TimeSeriesAnalyzeQuery_Granularity_value = map[string]int32{
		"GRANULARITY_NONE": 0,
		"DAILY":            1,
		"MONTHLY":          2,
		"YEARLY":           3,
	}
)

func (x TimeSeriesAnalyzeQuery_Granularity) Enum() *TimeSeriesAnalyzeQuery_Granularity {
	p := new(TimeSeriesAnalyzeQuery_Granularity)
	*p = x
	return p
}

func (x TimeSeriesAnalyzeQuery_Granularity) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (TimeSeriesAnalyzeQuery_Granularity) Descriptor() protoreflect.EnumDescriptor {
	return file_spaceone_api_core_v2_query_proto_enumTypes[0].Descriptor()
}

func (TimeSeriesAnalyzeQuery_Granularity) Type() protoreflect.EnumType {
	return &file_spaceone_api_core_v2_query_proto_enumTypes[0]
}

func (x TimeSeriesAnalyzeQuery_Granularity) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use TimeSeriesAnalyzeQuery_Granularity.Descriptor instead.
func (TimeSeriesAnalyzeQuery_Granularity) EnumDescriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{17, 0}
}

type ExportOption_QueryType int32

const (
	ExportOption_QUERY_TYPE_NONE ExportOption_QueryType = 0
	ExportOption_SEARCH          ExportOption_QueryType = 1
	ExportOption_ANALYZE         ExportOption_QueryType = 2
)

// Enum value maps for ExportOption_QueryType.
var (
	ExportOption_QueryType_name = map[int32]string{
		0: "QUERY_TYPE_NONE",
		1: "SEARCH",
		2: "ANALYZE",
	}
	ExportOption_QueryType_value = map[string]int32{
		"QUERY_TYPE_NONE": 0,
		"SEARCH":          1,
		"ANALYZE":         2,
	}
)

func (x ExportOption_QueryType) Enum() *ExportOption_QueryType {
	p := new(ExportOption_QueryType)
	*p = x
	return p
}

func (x ExportOption_QueryType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ExportOption_QueryType) Descriptor() protoreflect.EnumDescriptor {
	return file_spaceone_api_core_v2_query_proto_enumTypes[1].Descriptor()
}

func (ExportOption_QueryType) Type() protoreflect.EnumType {
	return &file_spaceone_api_core_v2_query_proto_enumTypes[1]
}

func (x ExportOption_QueryType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ExportOption_QueryType.Descriptor instead.
func (ExportOption_QueryType) EnumDescriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{21, 0}
}

type Filter struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to KeyAlias:
	//
	//	*Filter_Key
	//	*Filter_K
	KeyAlias isFilter_KeyAlias `protobuf_oneof:"key_alias"`
	// Types that are valid to be assigned to ValueAlias:
	//
	//	*Filter_Value
	//	*Filter_V
	ValueAlias isFilter_ValueAlias `protobuf_oneof:"value_alias"`
	// Types that are valid to be assigned to OperatorAlias:
	//
	//	*Filter_Operator
	//	*Filter_O
	OperatorAlias isFilter_OperatorAlias `protobuf_oneof:"operator_alias"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Filter) Reset() {
	*x = Filter{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Filter) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter) ProtoMessage() {}

func (x *Filter) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter.ProtoReflect.Descriptor instead.
func (*Filter) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{0}
}

func (x *Filter) GetKeyAlias() isFilter_KeyAlias {
	if x != nil {
		return x.KeyAlias
	}
	return nil
}

func (x *Filter) GetKey() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*Filter_Key); ok {
			return x.Key
		}
	}
	return ""
}

func (x *Filter) GetK() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*Filter_K); ok {
			return x.K
		}
	}
	return ""
}

func (x *Filter) GetValueAlias() isFilter_ValueAlias {
	if x != nil {
		return x.ValueAlias
	}
	return nil
}

func (x *Filter) GetValue() *_struct.Value {
	if x != nil {
		if x, ok := x.ValueAlias.(*Filter_Value); ok {
			return x.Value
		}
	}
	return nil
}

func (x *Filter) GetV() *_struct.Value {
	if x != nil {
		if x, ok := x.ValueAlias.(*Filter_V); ok {
			return x.V
		}
	}
	return nil
}

func (x *Filter) GetOperatorAlias() isFilter_OperatorAlias {
	if x != nil {
		return x.OperatorAlias
	}
	return nil
}

func (x *Filter) GetOperator() string {
	if x != nil {
		if x, ok := x.OperatorAlias.(*Filter_Operator); ok {
			return x.Operator
		}
	}
	return ""
}

func (x *Filter) GetO() string {
	if x != nil {
		if x, ok := x.OperatorAlias.(*Filter_O); ok {
			return x.O
		}
	}
	return ""
}

type isFilter_KeyAlias interface {
	isFilter_KeyAlias()
}

type Filter_Key struct {
	Key string `protobuf:"bytes,1,opt,name=key,proto3,oneof"`
}

type Filter_K struct {
	K string `protobuf:"bytes,2,opt,name=k,proto3,oneof"`
}

func (*Filter_Key) isFilter_KeyAlias() {}

func (*Filter_K) isFilter_KeyAlias() {}

type isFilter_ValueAlias interface {
	isFilter_ValueAlias()
}

type Filter_Value struct {
	Value *_struct.Value `protobuf:"bytes,3,opt,name=value,proto3,oneof"`
}

type Filter_V struct {
	V *_struct.Value `protobuf:"bytes,4,opt,name=v,proto3,oneof"`
}

func (*Filter_Value) isFilter_ValueAlias() {}

func (*Filter_V) isFilter_ValueAlias() {}

type isFilter_OperatorAlias interface {
	isFilter_OperatorAlias()
}

type Filter_Operator struct {
	Operator string `protobuf:"bytes,5,opt,name=operator,proto3,oneof"`
}

type Filter_O struct {
	O string `protobuf:"bytes,6,opt,name=o,proto3,oneof"`
}

func (*Filter_Operator) isFilter_OperatorAlias() {}

func (*Filter_O) isFilter_OperatorAlias() {}

type Sort struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Key           string                 `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	Desc          bool                   `protobuf:"varint,2,opt,name=desc,proto3" json:"desc,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Sort) Reset() {
	*x = Sort{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Sort) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Sort) ProtoMessage() {}

func (x *Sort) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Sort.ProtoReflect.Descriptor instead.
func (*Sort) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{1}
}

func (x *Sort) GetKey() string {
	if x != nil {
		return x.Key
	}
	return ""
}

func (x *Sort) GetDesc() bool {
	if x != nil {
		return x.Desc
	}
	return false
}

type Page struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Start         uint32                 `protobuf:"varint,1,opt,name=start,proto3" json:"start,omitempty"`
	Limit         uint32                 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Page) Reset() {
	*x = Page{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Page) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Page) ProtoMessage() {}

func (x *Page) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Page.ProtoReflect.Descriptor instead.
func (*Page) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{2}
}

func (x *Page) GetStart() uint32 {
	if x != nil {
		return x.Start
	}
	return 0
}

func (x *Page) GetLimit() uint32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

type Unwind struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Path          string                 `protobuf:"bytes,1,opt,name=path,proto3" json:"path,omitempty"`
	Filter        []*Filter              `protobuf:"bytes,2,rep,name=filter,proto3" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Unwind) Reset() {
	*x = Unwind{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Unwind) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Unwind) ProtoMessage() {}

func (x *Unwind) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Unwind.ProtoReflect.Descriptor instead.
func (*Unwind) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{3}
}

func (x *Unwind) GetPath() string {
	if x != nil {
		return x.Path
	}
	return ""
}

func (x *Unwind) GetFilter() []*Filter {
	if x != nil {
		return x.Filter
	}
	return nil
}

type Query struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Filter        []*Filter              `protobuf:"bytes,1,rep,name=filter,proto3" json:"filter,omitempty"`
	FilterOr      []*Filter              `protobuf:"bytes,2,rep,name=filter_or,json=filterOr,proto3" json:"filter_or,omitempty"`
	Sort          []*Sort                `protobuf:"bytes,3,rep,name=sort,proto3" json:"sort,omitempty"`
	Page          *Page                  `protobuf:"bytes,4,opt,name=page,proto3" json:"page,omitempty"`
	Minimal       bool                   `protobuf:"varint,5,opt,name=minimal,proto3" json:"minimal,omitempty"`
	CountOnly     bool                   `protobuf:"varint,6,opt,name=count_only,json=countOnly,proto3" json:"count_only,omitempty"`
	Only          []string               `protobuf:"bytes,7,rep,name=only,proto3" json:"only,omitempty"`
	Keyword       string                 `protobuf:"bytes,8,opt,name=keyword,proto3" json:"keyword,omitempty"`
	Unwind        *Unwind                `protobuf:"bytes,9,opt,name=unwind,proto3" json:"unwind,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Query) Reset() {
	*x = Query{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Query) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Query) ProtoMessage() {}

func (x *Query) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Query.ProtoReflect.Descriptor instead.
func (*Query) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{4}
}

func (x *Query) GetFilter() []*Filter {
	if x != nil {
		return x.Filter
	}
	return nil
}

func (x *Query) GetFilterOr() []*Filter {
	if x != nil {
		return x.FilterOr
	}
	return nil
}

func (x *Query) GetSort() []*Sort {
	if x != nil {
		return x.Sort
	}
	return nil
}

func (x *Query) GetPage() *Page {
	if x != nil {
		return x.Page
	}
	return nil
}

func (x *Query) GetMinimal() bool {
	if x != nil {
		return x.Minimal
	}
	return false
}

func (x *Query) GetCountOnly() bool {
	if x != nil {
		return x.CountOnly
	}
	return false
}

func (x *Query) GetOnly() []string {
	if x != nil {
		return x.Only
	}
	return nil
}

func (x *Query) GetKeyword() string {
	if x != nil {
		return x.Keyword
	}
	return ""
}

func (x *Query) GetUnwind() *Unwind {
	if x != nil {
		return x.Unwind
	}
	return nil
}

type AggregateGroupKey struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to KeyAlias:
	//
	//	*AggregateGroupKey_Key
	//	*AggregateGroupKey_K
	KeyAlias isAggregateGroupKey_KeyAlias `protobuf_oneof:"key_alias"`
	// Types that are valid to be assigned to NameAlias:
	//
	//	*AggregateGroupKey_Name
	//	*AggregateGroupKey_N
	NameAlias     isAggregateGroupKey_NameAlias `protobuf_oneof:"name_alias"`
	DateFormat    string                        `protobuf:"bytes,5,opt,name=date_format,json=dateFormat,proto3" json:"date_format,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AggregateGroupKey) Reset() {
	*x = AggregateGroupKey{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AggregateGroupKey) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AggregateGroupKey) ProtoMessage() {}

func (x *AggregateGroupKey) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AggregateGroupKey.ProtoReflect.Descriptor instead.
func (*AggregateGroupKey) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{5}
}

func (x *AggregateGroupKey) GetKeyAlias() isAggregateGroupKey_KeyAlias {
	if x != nil {
		return x.KeyAlias
	}
	return nil
}

func (x *AggregateGroupKey) GetKey() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateGroupKey_Key); ok {
			return x.Key
		}
	}
	return ""
}

func (x *AggregateGroupKey) GetK() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateGroupKey_K); ok {
			return x.K
		}
	}
	return ""
}

func (x *AggregateGroupKey) GetNameAlias() isAggregateGroupKey_NameAlias {
	if x != nil {
		return x.NameAlias
	}
	return nil
}

func (x *AggregateGroupKey) GetName() string {
	if x != nil {
		if x, ok := x.NameAlias.(*AggregateGroupKey_Name); ok {
			return x.Name
		}
	}
	return ""
}

func (x *AggregateGroupKey) GetN() string {
	if x != nil {
		if x, ok := x.NameAlias.(*AggregateGroupKey_N); ok {
			return x.N
		}
	}
	return ""
}

func (x *AggregateGroupKey) GetDateFormat() string {
	if x != nil {
		return x.DateFormat
	}
	return ""
}

type isAggregateGroupKey_KeyAlias interface {
	isAggregateGroupKey_KeyAlias()
}

type AggregateGroupKey_Key struct {
	Key string `protobuf:"bytes,1,opt,name=key,proto3,oneof"`
}

type AggregateGroupKey_K struct {
	K string `protobuf:"bytes,2,opt,name=k,proto3,oneof"`
}

func (*AggregateGroupKey_Key) isAggregateGroupKey_KeyAlias() {}

func (*AggregateGroupKey_K) isAggregateGroupKey_KeyAlias() {}

type isAggregateGroupKey_NameAlias interface {
	isAggregateGroupKey_NameAlias()
}

type AggregateGroupKey_Name struct {
	Name string `protobuf:"bytes,3,opt,name=name,proto3,oneof"`
}

type AggregateGroupKey_N struct {
	N string `protobuf:"bytes,4,opt,name=n,proto3,oneof"`
}

func (*AggregateGroupKey_Name) isAggregateGroupKey_NameAlias() {}

func (*AggregateGroupKey_N) isAggregateGroupKey_NameAlias() {}

type AggregateGroupSubField struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to KeyAlias:
	//
	//	*AggregateGroupSubField_Key
	//	*AggregateGroupSubField_K
	KeyAlias isAggregateGroupSubField_KeyAlias `protobuf_oneof:"key_alias"`
	// Types that are valid to be assigned to NameAlias:
	//
	//	*AggregateGroupSubField_Name
	//	*AggregateGroupSubField_N
	NameAlias     isAggregateGroupSubField_NameAlias `protobuf_oneof:"name_alias"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AggregateGroupSubField) Reset() {
	*x = AggregateGroupSubField{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AggregateGroupSubField) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AggregateGroupSubField) ProtoMessage() {}

func (x *AggregateGroupSubField) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AggregateGroupSubField.ProtoReflect.Descriptor instead.
func (*AggregateGroupSubField) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{6}
}

func (x *AggregateGroupSubField) GetKeyAlias() isAggregateGroupSubField_KeyAlias {
	if x != nil {
		return x.KeyAlias
	}
	return nil
}

func (x *AggregateGroupSubField) GetKey() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateGroupSubField_Key); ok {
			return x.Key
		}
	}
	return ""
}

func (x *AggregateGroupSubField) GetK() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateGroupSubField_K); ok {
			return x.K
		}
	}
	return ""
}

func (x *AggregateGroupSubField) GetNameAlias() isAggregateGroupSubField_NameAlias {
	if x != nil {
		return x.NameAlias
	}
	return nil
}

func (x *AggregateGroupSubField) GetName() string {
	if x != nil {
		if x, ok := x.NameAlias.(*AggregateGroupSubField_Name); ok {
			return x.Name
		}
	}
	return ""
}

func (x *AggregateGroupSubField) GetN() string {
	if x != nil {
		if x, ok := x.NameAlias.(*AggregateGroupSubField_N); ok {
			return x.N
		}
	}
	return ""
}

type isAggregateGroupSubField_KeyAlias interface {
	isAggregateGroupSubField_KeyAlias()
}

type AggregateGroupSubField_Key struct {
	Key string `protobuf:"bytes,1,opt,name=key,proto3,oneof"`
}

type AggregateGroupSubField_K struct {
	K string `protobuf:"bytes,2,opt,name=k,proto3,oneof"`
}

func (*AggregateGroupSubField_Key) isAggregateGroupSubField_KeyAlias() {}

func (*AggregateGroupSubField_K) isAggregateGroupSubField_KeyAlias() {}

type isAggregateGroupSubField_NameAlias interface {
	isAggregateGroupSubField_NameAlias()
}

type AggregateGroupSubField_Name struct {
	Name string `protobuf:"bytes,3,opt,name=name,proto3,oneof"`
}

type AggregateGroupSubField_N struct {
	N string `protobuf:"bytes,4,opt,name=n,proto3,oneof"`
}

func (*AggregateGroupSubField_Name) isAggregateGroupSubField_NameAlias() {}

func (*AggregateGroupSubField_N) isAggregateGroupSubField_NameAlias() {}

type AggregateSubCondition struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to KeyAlias:
	//
	//	*AggregateSubCondition_Key
	//	*AggregateSubCondition_K
	KeyAlias isAggregateSubCondition_KeyAlias `protobuf_oneof:"key_alias"`
	// Types that are valid to be assigned to ValueAlias:
	//
	//	*AggregateSubCondition_Value
	//	*AggregateSubCondition_V
	ValueAlias isAggregateSubCondition_ValueAlias `protobuf_oneof:"value_alias"`
	// Types that are valid to be assigned to OperatorAlias:
	//
	//	*AggregateSubCondition_Operator
	//	*AggregateSubCondition_O
	OperatorAlias isAggregateSubCondition_OperatorAlias `protobuf_oneof:"operator_alias"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AggregateSubCondition) Reset() {
	*x = AggregateSubCondition{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AggregateSubCondition) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AggregateSubCondition) ProtoMessage() {}

func (x *AggregateSubCondition) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AggregateSubCondition.ProtoReflect.Descriptor instead.
func (*AggregateSubCondition) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{7}
}

func (x *AggregateSubCondition) GetKeyAlias() isAggregateSubCondition_KeyAlias {
	if x != nil {
		return x.KeyAlias
	}
	return nil
}

func (x *AggregateSubCondition) GetKey() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateSubCondition_Key); ok {
			return x.Key
		}
	}
	return ""
}

func (x *AggregateSubCondition) GetK() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateSubCondition_K); ok {
			return x.K
		}
	}
	return ""
}

func (x *AggregateSubCondition) GetValueAlias() isAggregateSubCondition_ValueAlias {
	if x != nil {
		return x.ValueAlias
	}
	return nil
}

func (x *AggregateSubCondition) GetValue() *_struct.Value {
	if x != nil {
		if x, ok := x.ValueAlias.(*AggregateSubCondition_Value); ok {
			return x.Value
		}
	}
	return nil
}

func (x *AggregateSubCondition) GetV() *_struct.Value {
	if x != nil {
		if x, ok := x.ValueAlias.(*AggregateSubCondition_V); ok {
			return x.V
		}
	}
	return nil
}

func (x *AggregateSubCondition) GetOperatorAlias() isAggregateSubCondition_OperatorAlias {
	if x != nil {
		return x.OperatorAlias
	}
	return nil
}

func (x *AggregateSubCondition) GetOperator() string {
	if x != nil {
		if x, ok := x.OperatorAlias.(*AggregateSubCondition_Operator); ok {
			return x.Operator
		}
	}
	return ""
}

func (x *AggregateSubCondition) GetO() string {
	if x != nil {
		if x, ok := x.OperatorAlias.(*AggregateSubCondition_O); ok {
			return x.O
		}
	}
	return ""
}

type isAggregateSubCondition_KeyAlias interface {
	isAggregateSubCondition_KeyAlias()
}

type AggregateSubCondition_Key struct {
	Key string `protobuf:"bytes,1,opt,name=key,proto3,oneof"`
}

type AggregateSubCondition_K struct {
	K string `protobuf:"bytes,2,opt,name=k,proto3,oneof"`
}

func (*AggregateSubCondition_Key) isAggregateSubCondition_KeyAlias() {}

func (*AggregateSubCondition_K) isAggregateSubCondition_KeyAlias() {}

type isAggregateSubCondition_ValueAlias interface {
	isAggregateSubCondition_ValueAlias()
}

type AggregateSubCondition_Value struct {
	Value *_struct.Value `protobuf:"bytes,3,opt,name=value,proto3,oneof"`
}

type AggregateSubCondition_V struct {
	V *_struct.Value `protobuf:"bytes,4,opt,name=v,proto3,oneof"`
}

func (*AggregateSubCondition_Value) isAggregateSubCondition_ValueAlias() {}

func (*AggregateSubCondition_V) isAggregateSubCondition_ValueAlias() {}

type isAggregateSubCondition_OperatorAlias interface {
	isAggregateSubCondition_OperatorAlias()
}

type AggregateSubCondition_Operator struct {
	Operator string `protobuf:"bytes,5,opt,name=operator,proto3,oneof"`
}

type AggregateSubCondition_O struct {
	O string `protobuf:"bytes,6,opt,name=o,proto3,oneof"`
}

func (*AggregateSubCondition_Operator) isAggregateSubCondition_OperatorAlias() {}

func (*AggregateSubCondition_O) isAggregateSubCondition_OperatorAlias() {}

type AggregateGroupField struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to KeyAlias:
	//
	//	*AggregateGroupField_Key
	//	*AggregateGroupField_K
	KeyAlias isAggregateGroupField_KeyAlias `protobuf_oneof:"key_alias"`
	// Types that are valid to be assigned to NameAlias:
	//
	//	*AggregateGroupField_Name
	//	*AggregateGroupField_N
	NameAlias isAggregateGroupField_NameAlias `protobuf_oneof:"name_alias"`
	// Types that are valid to be assigned to OperatorAlias:
	//
	//	*AggregateGroupField_Operator
	//	*AggregateGroupField_O
	OperatorAlias isAggregateGroupField_OperatorAlias `protobuf_oneof:"operator_alias"`
	Fields        []*AggregateGroupSubField           `protobuf:"bytes,7,rep,name=fields,proto3" json:"fields,omitempty"`
	Conditions    []*AggregateSubCondition            `protobuf:"bytes,8,rep,name=conditions,proto3" json:"conditions,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AggregateGroupField) Reset() {
	*x = AggregateGroupField{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AggregateGroupField) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AggregateGroupField) ProtoMessage() {}

func (x *AggregateGroupField) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AggregateGroupField.ProtoReflect.Descriptor instead.
func (*AggregateGroupField) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{8}
}

func (x *AggregateGroupField) GetKeyAlias() isAggregateGroupField_KeyAlias {
	if x != nil {
		return x.KeyAlias
	}
	return nil
}

func (x *AggregateGroupField) GetKey() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateGroupField_Key); ok {
			return x.Key
		}
	}
	return ""
}

func (x *AggregateGroupField) GetK() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateGroupField_K); ok {
			return x.K
		}
	}
	return ""
}

func (x *AggregateGroupField) GetNameAlias() isAggregateGroupField_NameAlias {
	if x != nil {
		return x.NameAlias
	}
	return nil
}

func (x *AggregateGroupField) GetName() string {
	if x != nil {
		if x, ok := x.NameAlias.(*AggregateGroupField_Name); ok {
			return x.Name
		}
	}
	return ""
}

func (x *AggregateGroupField) GetN() string {
	if x != nil {
		if x, ok := x.NameAlias.(*AggregateGroupField_N); ok {
			return x.N
		}
	}
	return ""
}

func (x *AggregateGroupField) GetOperatorAlias() isAggregateGroupField_OperatorAlias {
	if x != nil {
		return x.OperatorAlias
	}
	return nil
}

func (x *AggregateGroupField) GetOperator() string {
	if x != nil {
		if x, ok := x.OperatorAlias.(*AggregateGroupField_Operator); ok {
			return x.Operator
		}
	}
	return ""
}

func (x *AggregateGroupField) GetO() string {
	if x != nil {
		if x, ok := x.OperatorAlias.(*AggregateGroupField_O); ok {
			return x.O
		}
	}
	return ""
}

func (x *AggregateGroupField) GetFields() []*AggregateGroupSubField {
	if x != nil {
		return x.Fields
	}
	return nil
}

func (x *AggregateGroupField) GetConditions() []*AggregateSubCondition {
	if x != nil {
		return x.Conditions
	}
	return nil
}

type isAggregateGroupField_KeyAlias interface {
	isAggregateGroupField_KeyAlias()
}

type AggregateGroupField_Key struct {
	Key string `protobuf:"bytes,1,opt,name=key,proto3,oneof"`
}

type AggregateGroupField_K struct {
	K string `protobuf:"bytes,2,opt,name=k,proto3,oneof"`
}

func (*AggregateGroupField_Key) isAggregateGroupField_KeyAlias() {}

func (*AggregateGroupField_K) isAggregateGroupField_KeyAlias() {}

type isAggregateGroupField_NameAlias interface {
	isAggregateGroupField_NameAlias()
}

type AggregateGroupField_Name struct {
	Name string `protobuf:"bytes,3,opt,name=name,proto3,oneof"`
}

type AggregateGroupField_N struct {
	N string `protobuf:"bytes,4,opt,name=n,proto3,oneof"`
}

func (*AggregateGroupField_Name) isAggregateGroupField_NameAlias() {}

func (*AggregateGroupField_N) isAggregateGroupField_NameAlias() {}

type isAggregateGroupField_OperatorAlias interface {
	isAggregateGroupField_OperatorAlias()
}

type AggregateGroupField_Operator struct {
	Operator string `protobuf:"bytes,5,opt,name=operator,proto3,oneof"`
}

type AggregateGroupField_O struct {
	O string `protobuf:"bytes,6,opt,name=o,proto3,oneof"`
}

func (*AggregateGroupField_Operator) isAggregateGroupField_OperatorAlias() {}

func (*AggregateGroupField_O) isAggregateGroupField_OperatorAlias() {}

type AggregateGroup struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Keys          []*AggregateGroupKey   `protobuf:"bytes,1,rep,name=keys,proto3" json:"keys,omitempty"`
	Fields        []*AggregateGroupField `protobuf:"bytes,2,rep,name=fields,proto3" json:"fields,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AggregateGroup) Reset() {
	*x = AggregateGroup{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AggregateGroup) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AggregateGroup) ProtoMessage() {}

func (x *AggregateGroup) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AggregateGroup.ProtoReflect.Descriptor instead.
func (*AggregateGroup) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{9}
}

func (x *AggregateGroup) GetKeys() []*AggregateGroupKey {
	if x != nil {
		return x.Keys
	}
	return nil
}

func (x *AggregateGroup) GetFields() []*AggregateGroupField {
	if x != nil {
		return x.Fields
	}
	return nil
}

type AggregateProjectField struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to KeyAlias:
	//
	//	*AggregateProjectField_Key
	//	*AggregateProjectField_K
	KeyAlias isAggregateProjectField_KeyAlias `protobuf_oneof:"key_alias"`
	// Types that are valid to be assigned to NameAlias:
	//
	//	*AggregateProjectField_Name
	//	*AggregateProjectField_N
	NameAlias isAggregateProjectField_NameAlias `protobuf_oneof:"name_alias"`
	// Types that are valid to be assigned to OperatorAlias:
	//
	//	*AggregateProjectField_Operator
	//	*AggregateProjectField_O
	OperatorAlias isAggregateProjectField_OperatorAlias `protobuf_oneof:"operator_alias"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AggregateProjectField) Reset() {
	*x = AggregateProjectField{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AggregateProjectField) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AggregateProjectField) ProtoMessage() {}

func (x *AggregateProjectField) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AggregateProjectField.ProtoReflect.Descriptor instead.
func (*AggregateProjectField) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{10}
}

func (x *AggregateProjectField) GetKeyAlias() isAggregateProjectField_KeyAlias {
	if x != nil {
		return x.KeyAlias
	}
	return nil
}

func (x *AggregateProjectField) GetKey() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateProjectField_Key); ok {
			return x.Key
		}
	}
	return ""
}

func (x *AggregateProjectField) GetK() string {
	if x != nil {
		if x, ok := x.KeyAlias.(*AggregateProjectField_K); ok {
			return x.K
		}
	}
	return ""
}

func (x *AggregateProjectField) GetNameAlias() isAggregateProjectField_NameAlias {
	if x != nil {
		return x.NameAlias
	}
	return nil
}

func (x *AggregateProjectField) GetName() string {
	if x != nil {
		if x, ok := x.NameAlias.(*AggregateProjectField_Name); ok {
			return x.Name
		}
	}
	return ""
}

func (x *AggregateProjectField) GetN() string {
	if x != nil {
		if x, ok := x.NameAlias.(*AggregateProjectField_N); ok {
			return x.N
		}
	}
	return ""
}

func (x *AggregateProjectField) GetOperatorAlias() isAggregateProjectField_OperatorAlias {
	if x != nil {
		return x.OperatorAlias
	}
	return nil
}

func (x *AggregateProjectField) GetOperator() string {
	if x != nil {
		if x, ok := x.OperatorAlias.(*AggregateProjectField_Operator); ok {
			return x.Operator
		}
	}
	return ""
}

func (x *AggregateProjectField) GetO() string {
	if x != nil {
		if x, ok := x.OperatorAlias.(*AggregateProjectField_O); ok {
			return x.O
		}
	}
	return ""
}

type isAggregateProjectField_KeyAlias interface {
	isAggregateProjectField_KeyAlias()
}

type AggregateProjectField_Key struct {
	Key string `protobuf:"bytes,1,opt,name=key,proto3,oneof"`
}

type AggregateProjectField_K struct {
	K string `protobuf:"bytes,2,opt,name=k,proto3,oneof"`
}

func (*AggregateProjectField_Key) isAggregateProjectField_KeyAlias() {}

func (*AggregateProjectField_K) isAggregateProjectField_KeyAlias() {}

type isAggregateProjectField_NameAlias interface {
	isAggregateProjectField_NameAlias()
}

type AggregateProjectField_Name struct {
	Name string `protobuf:"bytes,3,opt,name=name,proto3,oneof"`
}

type AggregateProjectField_N struct {
	N string `protobuf:"bytes,4,opt,name=n,proto3,oneof"`
}

func (*AggregateProjectField_Name) isAggregateProjectField_NameAlias() {}

func (*AggregateProjectField_N) isAggregateProjectField_NameAlias() {}

type isAggregateProjectField_OperatorAlias interface {
	isAggregateProjectField_OperatorAlias()
}

type AggregateProjectField_Operator struct {
	Operator string `protobuf:"bytes,5,opt,name=operator,proto3,oneof"`
}

type AggregateProjectField_O struct {
	O string `protobuf:"bytes,6,opt,name=o,proto3,oneof"`
}

func (*AggregateProjectField_Operator) isAggregateProjectField_OperatorAlias() {}

func (*AggregateProjectField_O) isAggregateProjectField_OperatorAlias() {}

type AggregateProject struct {
	state         protoimpl.MessageState   `protogen:"open.v1"`
	Fields        []*AggregateProjectField `protobuf:"bytes,1,rep,name=fields,proto3" json:"fields,omitempty"`
	ExcludeKeys   bool                     `protobuf:"varint,2,opt,name=exclude_keys,json=excludeKeys,proto3" json:"exclude_keys,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AggregateProject) Reset() {
	*x = AggregateProject{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AggregateProject) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AggregateProject) ProtoMessage() {}

func (x *AggregateProject) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AggregateProject.ProtoReflect.Descriptor instead.
func (*AggregateProject) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{11}
}

func (x *AggregateProject) GetFields() []*AggregateProjectField {
	if x != nil {
		return x.Fields
	}
	return nil
}

func (x *AggregateProject) GetExcludeKeys() bool {
	if x != nil {
		return x.ExcludeKeys
	}
	return false
}

type AggregateUnwind struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Path          string                 `protobuf:"bytes,1,opt,name=path,proto3" json:"path,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AggregateUnwind) Reset() {
	*x = AggregateUnwind{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AggregateUnwind) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AggregateUnwind) ProtoMessage() {}

func (x *AggregateUnwind) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AggregateUnwind.ProtoReflect.Descriptor instead.
func (*AggregateUnwind) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{12}
}

func (x *AggregateUnwind) GetPath() string {
	if x != nil {
		return x.Path
	}
	return ""
}

type AggregateCount struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AggregateCount) Reset() {
	*x = AggregateCount{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AggregateCount) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AggregateCount) ProtoMessage() {}

func (x *AggregateCount) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AggregateCount.ProtoReflect.Descriptor instead.
func (*AggregateCount) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{13}
}

func (x *AggregateCount) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type StatisticsAggregate struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to AggregateAlias:
	//
	//	*StatisticsAggregate_Unwind
	//	*StatisticsAggregate_Group
	//	*StatisticsAggregate_Count
	//	*StatisticsAggregate_Sort
	//	*StatisticsAggregate_Project
	//	*StatisticsAggregate_Limit
	//	*StatisticsAggregate_Skip
	AggregateAlias isStatisticsAggregate_AggregateAlias `protobuf_oneof:"aggregate_alias"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *StatisticsAggregate) Reset() {
	*x = StatisticsAggregate{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StatisticsAggregate) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StatisticsAggregate) ProtoMessage() {}

func (x *StatisticsAggregate) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StatisticsAggregate.ProtoReflect.Descriptor instead.
func (*StatisticsAggregate) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{14}
}

func (x *StatisticsAggregate) GetAggregateAlias() isStatisticsAggregate_AggregateAlias {
	if x != nil {
		return x.AggregateAlias
	}
	return nil
}

func (x *StatisticsAggregate) GetUnwind() *AggregateUnwind {
	if x != nil {
		if x, ok := x.AggregateAlias.(*StatisticsAggregate_Unwind); ok {
			return x.Unwind
		}
	}
	return nil
}

func (x *StatisticsAggregate) GetGroup() *AggregateGroup {
	if x != nil {
		if x, ok := x.AggregateAlias.(*StatisticsAggregate_Group); ok {
			return x.Group
		}
	}
	return nil
}

func (x *StatisticsAggregate) GetCount() *AggregateCount {
	if x != nil {
		if x, ok := x.AggregateAlias.(*StatisticsAggregate_Count); ok {
			return x.Count
		}
	}
	return nil
}

func (x *StatisticsAggregate) GetSort() *_struct.ListValue {
	if x != nil {
		if x, ok := x.AggregateAlias.(*StatisticsAggregate_Sort); ok {
			return x.Sort
		}
	}
	return nil
}

func (x *StatisticsAggregate) GetProject() *AggregateProject {
	if x != nil {
		if x, ok := x.AggregateAlias.(*StatisticsAggregate_Project); ok {
			return x.Project
		}
	}
	return nil
}

func (x *StatisticsAggregate) GetLimit() int32 {
	if x != nil {
		if x, ok := x.AggregateAlias.(*StatisticsAggregate_Limit); ok {
			return x.Limit
		}
	}
	return 0
}

func (x *StatisticsAggregate) GetSkip() int32 {
	if x != nil {
		if x, ok := x.AggregateAlias.(*StatisticsAggregate_Skip); ok {
			return x.Skip
		}
	}
	return 0
}

type isStatisticsAggregate_AggregateAlias interface {
	isStatisticsAggregate_AggregateAlias()
}

type StatisticsAggregate_Unwind struct {
	Unwind *AggregateUnwind `protobuf:"bytes,1,opt,name=unwind,proto3,oneof"`
}

type StatisticsAggregate_Group struct {
	Group *AggregateGroup `protobuf:"bytes,2,opt,name=group,proto3,oneof"`
}

type StatisticsAggregate_Count struct {
	Count *AggregateCount `protobuf:"bytes,3,opt,name=count,proto3,oneof"`
}

type StatisticsAggregate_Sort struct {
	Sort *_struct.ListValue `protobuf:"bytes,4,opt,name=sort,proto3,oneof"`
}

type StatisticsAggregate_Project struct {
	Project *AggregateProject `protobuf:"bytes,5,opt,name=project,proto3,oneof"`
}

type StatisticsAggregate_Limit struct {
	Limit int32 `protobuf:"varint,6,opt,name=limit,proto3,oneof"`
}

type StatisticsAggregate_Skip struct {
	Skip int32 `protobuf:"varint,7,opt,name=skip,proto3,oneof"`
}

func (*StatisticsAggregate_Unwind) isStatisticsAggregate_AggregateAlias() {}

func (*StatisticsAggregate_Group) isStatisticsAggregate_AggregateAlias() {}

func (*StatisticsAggregate_Count) isStatisticsAggregate_AggregateAlias() {}

func (*StatisticsAggregate_Sort) isStatisticsAggregate_AggregateAlias() {}

func (*StatisticsAggregate_Project) isStatisticsAggregate_AggregateAlias() {}

func (*StatisticsAggregate_Limit) isStatisticsAggregate_AggregateAlias() {}

func (*StatisticsAggregate_Skip) isStatisticsAggregate_AggregateAlias() {}

type StatisticsQuery struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Filter        []*Filter              `protobuf:"bytes,1,rep,name=filter,proto3" json:"filter,omitempty"`
	FilterOr      []*Filter              `protobuf:"bytes,2,rep,name=filter_or,json=filterOr,proto3" json:"filter_or,omitempty"`
	Aggregate     []*StatisticsAggregate `protobuf:"bytes,3,rep,name=aggregate,proto3" json:"aggregate,omitempty"`
	Page          *Page                  `protobuf:"bytes,4,opt,name=page,proto3" json:"page,omitempty"`
	Distinct      string                 `protobuf:"bytes,5,opt,name=distinct,proto3" json:"distinct,omitempty"`
	Keyword       string                 `protobuf:"bytes,6,opt,name=keyword,proto3" json:"keyword,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StatisticsQuery) Reset() {
	*x = StatisticsQuery{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StatisticsQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StatisticsQuery) ProtoMessage() {}

func (x *StatisticsQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StatisticsQuery.ProtoReflect.Descriptor instead.
func (*StatisticsQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{15}
}

func (x *StatisticsQuery) GetFilter() []*Filter {
	if x != nil {
		return x.Filter
	}
	return nil
}

func (x *StatisticsQuery) GetFilterOr() []*Filter {
	if x != nil {
		return x.FilterOr
	}
	return nil
}

func (x *StatisticsQuery) GetAggregate() []*StatisticsAggregate {
	if x != nil {
		return x.Aggregate
	}
	return nil
}

func (x *StatisticsQuery) GetPage() *Page {
	if x != nil {
		return x.Page
	}
	return nil
}

func (x *StatisticsQuery) GetDistinct() string {
	if x != nil {
		return x.Distinct
	}
	return ""
}

func (x *StatisticsQuery) GetKeyword() string {
	if x != nil {
		return x.Keyword
	}
	return ""
}

type AnalyzeUnwind struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Path          string                 `protobuf:"bytes,1,opt,name=path,proto3" json:"path,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AnalyzeUnwind) Reset() {
	*x = AnalyzeUnwind{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AnalyzeUnwind) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AnalyzeUnwind) ProtoMessage() {}

func (x *AnalyzeUnwind) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AnalyzeUnwind.ProtoReflect.Descriptor instead.
func (*AnalyzeUnwind) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{16}
}

func (x *AnalyzeUnwind) GetPath() string {
	if x != nil {
		return x.Path
	}
	return ""
}

type TimeSeriesAnalyzeQuery struct {
	state         protoimpl.MessageState             `protogen:"open.v1"`
	Granularity   TimeSeriesAnalyzeQuery_Granularity `protobuf:"varint,1,opt,name=granularity,proto3,enum=spaceone.api.core.v2.TimeSeriesAnalyzeQuery_Granularity" json:"granularity,omitempty"`
	Start         string                             `protobuf:"bytes,2,opt,name=start,proto3" json:"start,omitempty"`
	End           string                             `protobuf:"bytes,3,opt,name=end,proto3" json:"end,omitempty"`
	GroupBy       *_struct.ListValue                 `protobuf:"bytes,4,opt,name=group_by,json=groupBy,proto3" json:"group_by,omitempty"`
	FieldGroup    []string                           `protobuf:"bytes,5,rep,name=field_group,json=fieldGroup,proto3" json:"field_group,omitempty"`
	Filter        []*Filter                          `protobuf:"bytes,6,rep,name=filter,proto3" json:"filter,omitempty"`
	FilterOr      []*Filter                          `protobuf:"bytes,7,rep,name=filter_or,json=filterOr,proto3" json:"filter_or,omitempty"`
	Page          *Page                              `protobuf:"bytes,8,opt,name=page,proto3" json:"page,omitempty"`
	Sort          []*Sort                            `protobuf:"bytes,9,rep,name=sort,proto3" json:"sort,omitempty"`
	Fields        *_struct.Struct                    `protobuf:"bytes,10,opt,name=fields,proto3" json:"fields,omitempty"`
	Select        *_struct.Struct                    `protobuf:"bytes,11,opt,name=select,proto3" json:"select,omitempty"`
	Keyword       string                             `protobuf:"bytes,12,opt,name=keyword,proto3" json:"keyword,omitempty"`
	Unwind        *AnalyzeUnwind                     `protobuf:"bytes,13,opt,name=unwind,proto3" json:"unwind,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TimeSeriesAnalyzeQuery) Reset() {
	*x = TimeSeriesAnalyzeQuery{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TimeSeriesAnalyzeQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TimeSeriesAnalyzeQuery) ProtoMessage() {}

func (x *TimeSeriesAnalyzeQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TimeSeriesAnalyzeQuery.ProtoReflect.Descriptor instead.
func (*TimeSeriesAnalyzeQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{17}
}

func (x *TimeSeriesAnalyzeQuery) GetGranularity() TimeSeriesAnalyzeQuery_Granularity {
	if x != nil {
		return x.Granularity
	}
	return TimeSeriesAnalyzeQuery_GRANULARITY_NONE
}

func (x *TimeSeriesAnalyzeQuery) GetStart() string {
	if x != nil {
		return x.Start
	}
	return ""
}

func (x *TimeSeriesAnalyzeQuery) GetEnd() string {
	if x != nil {
		return x.End
	}
	return ""
}

func (x *TimeSeriesAnalyzeQuery) GetGroupBy() *_struct.ListValue {
	if x != nil {
		return x.GroupBy
	}
	return nil
}

func (x *TimeSeriesAnalyzeQuery) GetFieldGroup() []string {
	if x != nil {
		return x.FieldGroup
	}
	return nil
}

func (x *TimeSeriesAnalyzeQuery) GetFilter() []*Filter {
	if x != nil {
		return x.Filter
	}
	return nil
}

func (x *TimeSeriesAnalyzeQuery) GetFilterOr() []*Filter {
	if x != nil {
		return x.FilterOr
	}
	return nil
}

func (x *TimeSeriesAnalyzeQuery) GetPage() *Page {
	if x != nil {
		return x.Page
	}
	return nil
}

func (x *TimeSeriesAnalyzeQuery) GetSort() []*Sort {
	if x != nil {
		return x.Sort
	}
	return nil
}

func (x *TimeSeriesAnalyzeQuery) GetFields() *_struct.Struct {
	if x != nil {
		return x.Fields
	}
	return nil
}

func (x *TimeSeriesAnalyzeQuery) GetSelect() *_struct.Struct {
	if x != nil {
		return x.Select
	}
	return nil
}

func (x *TimeSeriesAnalyzeQuery) GetKeyword() string {
	if x != nil {
		return x.Keyword
	}
	return ""
}

func (x *TimeSeriesAnalyzeQuery) GetUnwind() *AnalyzeUnwind {
	if x != nil {
		return x.Unwind
	}
	return nil
}

type AnalyzeQuery struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GroupBy       *_struct.ListValue     `protobuf:"bytes,1,opt,name=group_by,json=groupBy,proto3" json:"group_by,omitempty"`
	FieldGroup    []string               `protobuf:"bytes,2,rep,name=field_group,json=fieldGroup,proto3" json:"field_group,omitempty"`
	Filter        []*Filter              `protobuf:"bytes,3,rep,name=filter,proto3" json:"filter,omitempty"`
	FilterOr      []*Filter              `protobuf:"bytes,4,rep,name=filter_or,json=filterOr,proto3" json:"filter_or,omitempty"`
	Page          *Page                  `protobuf:"bytes,5,opt,name=page,proto3" json:"page,omitempty"`
	Sort          []*Sort                `protobuf:"bytes,6,rep,name=sort,proto3" json:"sort,omitempty"`
	Fields        *_struct.Struct        `protobuf:"bytes,7,opt,name=fields,proto3" json:"fields,omitempty"`
	Select        *_struct.Struct        `protobuf:"bytes,8,opt,name=select,proto3" json:"select,omitempty"`
	Keyword       string                 `protobuf:"bytes,9,opt,name=keyword,proto3" json:"keyword,omitempty"`
	Unwind        *AnalyzeUnwind         `protobuf:"bytes,10,opt,name=unwind,proto3" json:"unwind,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AnalyzeQuery) Reset() {
	*x = AnalyzeQuery{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AnalyzeQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AnalyzeQuery) ProtoMessage() {}

func (x *AnalyzeQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AnalyzeQuery.ProtoReflect.Descriptor instead.
func (*AnalyzeQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{18}
}

func (x *AnalyzeQuery) GetGroupBy() *_struct.ListValue {
	if x != nil {
		return x.GroupBy
	}
	return nil
}

func (x *AnalyzeQuery) GetFieldGroup() []string {
	if x != nil {
		return x.FieldGroup
	}
	return nil
}

func (x *AnalyzeQuery) GetFilter() []*Filter {
	if x != nil {
		return x.Filter
	}
	return nil
}

func (x *AnalyzeQuery) GetFilterOr() []*Filter {
	if x != nil {
		return x.FilterOr
	}
	return nil
}

func (x *AnalyzeQuery) GetPage() *Page {
	if x != nil {
		return x.Page
	}
	return nil
}

func (x *AnalyzeQuery) GetSort() []*Sort {
	if x != nil {
		return x.Sort
	}
	return nil
}

func (x *AnalyzeQuery) GetFields() *_struct.Struct {
	if x != nil {
		return x.Fields
	}
	return nil
}

func (x *AnalyzeQuery) GetSelect() *_struct.Struct {
	if x != nil {
		return x.Select
	}
	return nil
}

func (x *AnalyzeQuery) GetKeyword() string {
	if x != nil {
		return x.Keyword
	}
	return ""
}

func (x *AnalyzeQuery) GetUnwind() *AnalyzeUnwind {
	if x != nil {
		return x.Unwind
	}
	return nil
}

type ExportSearchQuery struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Filter        []*Filter              `protobuf:"bytes,1,rep,name=filter,proto3" json:"filter,omitempty"`
	FilterOr      []*Filter              `protobuf:"bytes,2,rep,name=filter_or,json=filterOr,proto3" json:"filter_or,omitempty"`
	Keyword       string                 `protobuf:"bytes,3,opt,name=keyword,proto3" json:"keyword,omitempty"`
	Sort          []*Sort                `protobuf:"bytes,4,rep,name=sort,proto3" json:"sort,omitempty"`
	Fields        *_struct.ListValue     `protobuf:"bytes,5,opt,name=fields,proto3" json:"fields,omitempty"`
	Unwind        *Unwind                `protobuf:"bytes,6,opt,name=unwind,proto3" json:"unwind,omitempty"`
	Page          *Page                  `protobuf:"bytes,7,opt,name=page,proto3" json:"page,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ExportSearchQuery) Reset() {
	*x = ExportSearchQuery{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExportSearchQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExportSearchQuery) ProtoMessage() {}

func (x *ExportSearchQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExportSearchQuery.ProtoReflect.Descriptor instead.
func (*ExportSearchQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{19}
}

func (x *ExportSearchQuery) GetFilter() []*Filter {
	if x != nil {
		return x.Filter
	}
	return nil
}

func (x *ExportSearchQuery) GetFilterOr() []*Filter {
	if x != nil {
		return x.FilterOr
	}
	return nil
}

func (x *ExportSearchQuery) GetKeyword() string {
	if x != nil {
		return x.Keyword
	}
	return ""
}

func (x *ExportSearchQuery) GetSort() []*Sort {
	if x != nil {
		return x.Sort
	}
	return nil
}

func (x *ExportSearchQuery) GetFields() *_struct.ListValue {
	if x != nil {
		return x.Fields
	}
	return nil
}

func (x *ExportSearchQuery) GetUnwind() *Unwind {
	if x != nil {
		return x.Unwind
	}
	return nil
}

func (x *ExportSearchQuery) GetPage() *Page {
	if x != nil {
		return x.Page
	}
	return nil
}

type ExportAnalyzeQuery struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Filter        []*Filter              `protobuf:"bytes,1,rep,name=filter,proto3" json:"filter,omitempty"`
	FilterOr      []*Filter              `protobuf:"bytes,2,rep,name=filter_or,json=filterOr,proto3" json:"filter_or,omitempty"`
	Keyword       string                 `protobuf:"bytes,3,opt,name=keyword,proto3" json:"keyword,omitempty"`
	Sort          []*Sort                `protobuf:"bytes,4,rep,name=sort,proto3" json:"sort,omitempty"`
	GroupBy       *_struct.ListValue     `protobuf:"bytes,5,opt,name=group_by,json=groupBy,proto3" json:"group_by,omitempty"`
	Fields        *_struct.Struct        `protobuf:"bytes,6,opt,name=fields,proto3" json:"fields,omitempty"`
	Select        *_struct.Struct        `protobuf:"bytes,7,opt,name=select,proto3" json:"select,omitempty"`
	Page          *Page                  `protobuf:"bytes,8,opt,name=page,proto3" json:"page,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ExportAnalyzeQuery) Reset() {
	*x = ExportAnalyzeQuery{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExportAnalyzeQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExportAnalyzeQuery) ProtoMessage() {}

func (x *ExportAnalyzeQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExportAnalyzeQuery.ProtoReflect.Descriptor instead.
func (*ExportAnalyzeQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{20}
}

func (x *ExportAnalyzeQuery) GetFilter() []*Filter {
	if x != nil {
		return x.Filter
	}
	return nil
}

func (x *ExportAnalyzeQuery) GetFilterOr() []*Filter {
	if x != nil {
		return x.FilterOr
	}
	return nil
}

func (x *ExportAnalyzeQuery) GetKeyword() string {
	if x != nil {
		return x.Keyword
	}
	return ""
}

func (x *ExportAnalyzeQuery) GetSort() []*Sort {
	if x != nil {
		return x.Sort
	}
	return nil
}

func (x *ExportAnalyzeQuery) GetGroupBy() *_struct.ListValue {
	if x != nil {
		return x.GroupBy
	}
	return nil
}

func (x *ExportAnalyzeQuery) GetFields() *_struct.Struct {
	if x != nil {
		return x.Fields
	}
	return nil
}

func (x *ExportAnalyzeQuery) GetSelect() *_struct.Struct {
	if x != nil {
		return x.Select
	}
	return nil
}

func (x *ExportAnalyzeQuery) GetPage() *Page {
	if x != nil {
		return x.Page
	}
	return nil
}

type ExportOption struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	Name      string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Title     string                 `protobuf:"bytes,2,opt,name=title,proto3" json:"title,omitempty"`
	QueryType ExportOption_QueryType `protobuf:"varint,11,opt,name=query_type,json=queryType,proto3,enum=spaceone.api.core.v2.ExportOption_QueryType" json:"query_type,omitempty"`
	// Types that are valid to be assigned to ExportQuery:
	//
	//	*ExportOption_SearchQuery
	//	*ExportOption_AnalyzeQuery
	ExportQuery   isExportOption_ExportQuery `protobuf_oneof:"export_query"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ExportOption) Reset() {
	*x = ExportOption{}
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExportOption) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExportOption) ProtoMessage() {}

func (x *ExportOption) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_core_v2_query_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExportOption.ProtoReflect.Descriptor instead.
func (*ExportOption) Descriptor() ([]byte, []int) {
	return file_spaceone_api_core_v2_query_proto_rawDescGZIP(), []int{21}
}

func (x *ExportOption) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ExportOption) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

func (x *ExportOption) GetQueryType() ExportOption_QueryType {
	if x != nil {
		return x.QueryType
	}
	return ExportOption_QUERY_TYPE_NONE
}

func (x *ExportOption) GetExportQuery() isExportOption_ExportQuery {
	if x != nil {
		return x.ExportQuery
	}
	return nil
}

func (x *ExportOption) GetSearchQuery() *ExportSearchQuery {
	if x != nil {
		if x, ok := x.ExportQuery.(*ExportOption_SearchQuery); ok {
			return x.SearchQuery
		}
	}
	return nil
}

func (x *ExportOption) GetAnalyzeQuery() *ExportAnalyzeQuery {
	if x != nil {
		if x, ok := x.ExportQuery.(*ExportOption_AnalyzeQuery); ok {
			return x.AnalyzeQuery
		}
	}
	return nil
}

type isExportOption_ExportQuery interface {
	isExportOption_ExportQuery()
}

type ExportOption_SearchQuery struct {
	SearchQuery *ExportSearchQuery `protobuf:"bytes,12,opt,name=search_query,json=searchQuery,proto3,oneof"`
}

type ExportOption_AnalyzeQuery struct {
	AnalyzeQuery *ExportAnalyzeQuery `protobuf:"bytes,13,opt,name=analyze_query,json=analyzeQuery,proto3,oneof"`
}

func (*ExportOption_SearchQuery) isExportOption_ExportQuery() {}

func (*ExportOption_AnalyzeQuery) isExportOption_ExportQuery() {}

var File_spaceone_api_core_v2_query_proto protoreflect.FileDescriptor

const file_spaceone_api_core_v2_query_proto_rawDesc = "" +
	"\n" +
	" spaceone/api/core/v2/query.proto\x12\x14spaceone.api.core.v2\x1a\x1cgoogle/protobuf/struct.proto\"\xe0\x01\n" +
	"\x06Filter\x12\x12\n" +
	"\x03key\x18\x01 \x01(\tH\x00R\x03key\x12\x0e\n" +
	"\x01k\x18\x02 \x01(\tH\x00R\x01k\x12.\n" +
	"\x05value\x18\x03 \x01(\v2\x16.google.protobuf.ValueH\x01R\x05value\x12&\n" +
	"\x01v\x18\x04 \x01(\v2\x16.google.protobuf.ValueH\x01R\x01v\x12\x1c\n" +
	"\boperator\x18\x05 \x01(\tH\x02R\boperator\x12\x0e\n" +
	"\x01o\x18\x06 \x01(\tH\x02R\x01oB\v\n" +
	"\tkey_aliasB\r\n" +
	"\vvalue_aliasB\x10\n" +
	"\x0eoperator_alias\",\n" +
	"\x04Sort\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x12\n" +
	"\x04desc\x18\x02 \x01(\bR\x04desc\"2\n" +
	"\x04Page\x12\x14\n" +
	"\x05start\x18\x01 \x01(\rR\x05start\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\rR\x05limit\"R\n" +
	"\x06Unwind\x12\x12\n" +
	"\x04path\x18\x01 \x01(\tR\x04path\x124\n" +
	"\x06filter\x18\x02 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\x06filter\"\xf5\x02\n" +
	"\x05Query\x124\n" +
	"\x06filter\x18\x01 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\x06filter\x129\n" +
	"\tfilter_or\x18\x02 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\bfilterOr\x12.\n" +
	"\x04sort\x18\x03 \x03(\v2\x1a.spaceone.api.core.v2.SortR\x04sort\x12.\n" +
	"\x04page\x18\x04 \x01(\v2\x1a.spaceone.api.core.v2.PageR\x04page\x12\x18\n" +
	"\aminimal\x18\x05 \x01(\bR\aminimal\x12\x1d\n" +
	"\n" +
	"count_only\x18\x06 \x01(\bR\tcountOnly\x12\x12\n" +
	"\x04only\x18\a \x03(\tR\x04only\x12\x18\n" +
	"\akeyword\x18\b \x01(\tR\akeyword\x124\n" +
	"\x06unwind\x18\t \x01(\v2\x1c.spaceone.api.core.v2.UnwindR\x06unwind\"\x99\x01\n" +
	"\x11AggregateGroupKey\x12\x12\n" +
	"\x03key\x18\x01 \x01(\tH\x00R\x03key\x12\x0e\n" +
	"\x01k\x18\x02 \x01(\tH\x00R\x01k\x12\x14\n" +
	"\x04name\x18\x03 \x01(\tH\x01R\x04name\x12\x0e\n" +
	"\x01n\x18\x04 \x01(\tH\x01R\x01n\x12\x1f\n" +
	"\vdate_format\x18\x05 \x01(\tR\n" +
	"dateFormatB\v\n" +
	"\tkey_aliasB\f\n" +
	"\n" +
	"name_alias\"}\n" +
	"\x16AggregateGroupSubField\x12\x12\n" +
	"\x03key\x18\x01 \x01(\tH\x00R\x03key\x12\x0e\n" +
	"\x01k\x18\x02 \x01(\tH\x00R\x01k\x12\x14\n" +
	"\x04name\x18\x03 \x01(\tH\x01R\x04name\x12\x0e\n" +
	"\x01n\x18\x04 \x01(\tH\x01R\x01nB\v\n" +
	"\tkey_aliasB\f\n" +
	"\n" +
	"name_alias\"\xef\x01\n" +
	"\x15AggregateSubCondition\x12\x12\n" +
	"\x03key\x18\x01 \x01(\tH\x00R\x03key\x12\x0e\n" +
	"\x01k\x18\x02 \x01(\tH\x00R\x01k\x12.\n" +
	"\x05value\x18\x03 \x01(\v2\x16.google.protobuf.ValueH\x01R\x05value\x12&\n" +
	"\x01v\x18\x04 \x01(\v2\x16.google.protobuf.ValueH\x01R\x01v\x12\x1c\n" +
	"\boperator\x18\x05 \x01(\tH\x02R\boperator\x12\x0e\n" +
	"\x01o\x18\x06 \x01(\tH\x02R\x01oB\v\n" +
	"\tkey_aliasB\r\n" +
	"\vvalue_aliasB\x10\n" +
	"\x0eoperator_alias\"\xcd\x02\n" +
	"\x13AggregateGroupField\x12\x12\n" +
	"\x03key\x18\x01 \x01(\tH\x00R\x03key\x12\x0e\n" +
	"\x01k\x18\x02 \x01(\tH\x00R\x01k\x12\x14\n" +
	"\x04name\x18\x03 \x01(\tH\x01R\x04name\x12\x0e\n" +
	"\x01n\x18\x04 \x01(\tH\x01R\x01n\x12\x1c\n" +
	"\boperator\x18\x05 \x01(\tH\x02R\boperator\x12\x0e\n" +
	"\x01o\x18\x06 \x01(\tH\x02R\x01o\x12D\n" +
	"\x06fields\x18\a \x03(\v2,.spaceone.api.core.v2.AggregateGroupSubFieldR\x06fields\x12K\n" +
	"\n" +
	"conditions\x18\b \x03(\v2+.spaceone.api.core.v2.AggregateSubConditionR\n" +
	"conditionsB\v\n" +
	"\tkey_aliasB\f\n" +
	"\n" +
	"name_aliasB\x10\n" +
	"\x0eoperator_alias\"\x90\x01\n" +
	"\x0eAggregateGroup\x12;\n" +
	"\x04keys\x18\x01 \x03(\v2'.spaceone.api.core.v2.AggregateGroupKeyR\x04keys\x12A\n" +
	"\x06fields\x18\x02 \x03(\v2).spaceone.api.core.v2.AggregateGroupFieldR\x06fields\"\xbc\x01\n" +
	"\x15AggregateProjectField\x12\x12\n" +
	"\x03key\x18\x01 \x01(\tH\x00R\x03key\x12\x0e\n" +
	"\x01k\x18\x02 \x01(\tH\x00R\x01k\x12\x14\n" +
	"\x04name\x18\x03 \x01(\tH\x01R\x04name\x12\x0e\n" +
	"\x01n\x18\x04 \x01(\tH\x01R\x01n\x12\x1c\n" +
	"\boperator\x18\x05 \x01(\tH\x02R\boperator\x12\x0e\n" +
	"\x01o\x18\x06 \x01(\tH\x02R\x01oB\v\n" +
	"\tkey_aliasB\f\n" +
	"\n" +
	"name_aliasB\x10\n" +
	"\x0eoperator_alias\"z\n" +
	"\x10AggregateProject\x12C\n" +
	"\x06fields\x18\x01 \x03(\v2+.spaceone.api.core.v2.AggregateProjectFieldR\x06fields\x12!\n" +
	"\fexclude_keys\x18\x02 \x01(\bR\vexcludeKeys\"%\n" +
	"\x0fAggregateUnwind\x12\x12\n" +
	"\x04path\x18\x01 \x01(\tR\x04path\"$\n" +
	"\x0eAggregateCount\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\"\x89\x03\n" +
	"\x13StatisticsAggregate\x12?\n" +
	"\x06unwind\x18\x01 \x01(\v2%.spaceone.api.core.v2.AggregateUnwindH\x00R\x06unwind\x12<\n" +
	"\x05group\x18\x02 \x01(\v2$.spaceone.api.core.v2.AggregateGroupH\x00R\x05group\x12<\n" +
	"\x05count\x18\x03 \x01(\v2$.spaceone.api.core.v2.AggregateCountH\x00R\x05count\x120\n" +
	"\x04sort\x18\x04 \x01(\v2\x1a.google.protobuf.ListValueH\x00R\x04sort\x12B\n" +
	"\aproject\x18\x05 \x01(\v2&.spaceone.api.core.v2.AggregateProjectH\x00R\aproject\x12\x16\n" +
	"\x05limit\x18\x06 \x01(\x05H\x00R\x05limit\x12\x14\n" +
	"\x04skip\x18\a \x01(\x05H\x00R\x04skipB\x11\n" +
	"\x0faggregate_alias\"\xb1\x02\n" +
	"\x0fStatisticsQuery\x124\n" +
	"\x06filter\x18\x01 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\x06filter\x129\n" +
	"\tfilter_or\x18\x02 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\bfilterOr\x12G\n" +
	"\taggregate\x18\x03 \x03(\v2).spaceone.api.core.v2.StatisticsAggregateR\taggregate\x12.\n" +
	"\x04page\x18\x04 \x01(\v2\x1a.spaceone.api.core.v2.PageR\x04page\x12\x1a\n" +
	"\bdistinct\x18\x05 \x01(\tR\bdistinct\x12\x18\n" +
	"\akeyword\x18\x06 \x01(\tR\akeyword\"#\n" +
	"\rAnalyzeUnwind\x12\x12\n" +
	"\x04path\x18\x01 \x01(\tR\x04path\"\xc7\x05\n" +
	"\x16TimeSeriesAnalyzeQuery\x12Z\n" +
	"\vgranularity\x18\x01 \x01(\x0e28.spaceone.api.core.v2.TimeSeriesAnalyzeQuery.GranularityR\vgranularity\x12\x14\n" +
	"\x05start\x18\x02 \x01(\tR\x05start\x12\x10\n" +
	"\x03end\x18\x03 \x01(\tR\x03end\x125\n" +
	"\bgroup_by\x18\x04 \x01(\v2\x1a.google.protobuf.ListValueR\agroupBy\x12\x1f\n" +
	"\vfield_group\x18\x05 \x03(\tR\n" +
	"fieldGroup\x124\n" +
	"\x06filter\x18\x06 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\x06filter\x129\n" +
	"\tfilter_or\x18\a \x03(\v2\x1c.spaceone.api.core.v2.FilterR\bfilterOr\x12.\n" +
	"\x04page\x18\b \x01(\v2\x1a.spaceone.api.core.v2.PageR\x04page\x12.\n" +
	"\x04sort\x18\t \x03(\v2\x1a.spaceone.api.core.v2.SortR\x04sort\x12/\n" +
	"\x06fields\x18\n" +
	" \x01(\v2\x17.google.protobuf.StructR\x06fields\x12/\n" +
	"\x06select\x18\v \x01(\v2\x17.google.protobuf.StructR\x06select\x12\x18\n" +
	"\akeyword\x18\f \x01(\tR\akeyword\x12;\n" +
	"\x06unwind\x18\r \x01(\v2#.spaceone.api.core.v2.AnalyzeUnwindR\x06unwind\"G\n" +
	"\vGranularity\x12\x14\n" +
	"\x10GRANULARITY_NONE\x10\x00\x12\t\n" +
	"\x05DAILY\x10\x01\x12\v\n" +
	"\aMONTHLY\x10\x02\x12\n" +
	"\n" +
	"\x06YEARLY\x10\x03\"\xf0\x03\n" +
	"\fAnalyzeQuery\x125\n" +
	"\bgroup_by\x18\x01 \x01(\v2\x1a.google.protobuf.ListValueR\agroupBy\x12\x1f\n" +
	"\vfield_group\x18\x02 \x03(\tR\n" +
	"fieldGroup\x124\n" +
	"\x06filter\x18\x03 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\x06filter\x129\n" +
	"\tfilter_or\x18\x04 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\bfilterOr\x12.\n" +
	"\x04page\x18\x05 \x01(\v2\x1a.spaceone.api.core.v2.PageR\x04page\x12.\n" +
	"\x04sort\x18\x06 \x03(\v2\x1a.spaceone.api.core.v2.SortR\x04sort\x12/\n" +
	"\x06fields\x18\a \x01(\v2\x17.google.protobuf.StructR\x06fields\x12/\n" +
	"\x06select\x18\b \x01(\v2\x17.google.protobuf.StructR\x06select\x12\x18\n" +
	"\akeyword\x18\t \x01(\tR\akeyword\x12;\n" +
	"\x06unwind\x18\n" +
	" \x01(\v2#.spaceone.api.core.v2.AnalyzeUnwindR\x06unwind\"\xe8\x02\n" +
	"\x11ExportSearchQuery\x124\n" +
	"\x06filter\x18\x01 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\x06filter\x129\n" +
	"\tfilter_or\x18\x02 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\bfilterOr\x12\x18\n" +
	"\akeyword\x18\x03 \x01(\tR\akeyword\x12.\n" +
	"\x04sort\x18\x04 \x03(\v2\x1a.spaceone.api.core.v2.SortR\x04sort\x122\n" +
	"\x06fields\x18\x05 \x01(\v2\x1a.google.protobuf.ListValueR\x06fields\x124\n" +
	"\x06unwind\x18\x06 \x01(\v2\x1c.spaceone.api.core.v2.UnwindR\x06unwind\x12.\n" +
	"\x04page\x18\a \x01(\v2\x1a.spaceone.api.core.v2.PageR\x04page\"\x98\x03\n" +
	"\x12ExportAnalyzeQuery\x124\n" +
	"\x06filter\x18\x01 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\x06filter\x129\n" +
	"\tfilter_or\x18\x02 \x03(\v2\x1c.spaceone.api.core.v2.FilterR\bfilterOr\x12\x18\n" +
	"\akeyword\x18\x03 \x01(\tR\akeyword\x12.\n" +
	"\x04sort\x18\x04 \x03(\v2\x1a.spaceone.api.core.v2.SortR\x04sort\x125\n" +
	"\bgroup_by\x18\x05 \x01(\v2\x1a.google.protobuf.ListValueR\agroupBy\x12/\n" +
	"\x06fields\x18\x06 \x01(\v2\x17.google.protobuf.StructR\x06fields\x12/\n" +
	"\x06select\x18\a \x01(\v2\x17.google.protobuf.StructR\x06select\x12.\n" +
	"\x04page\x18\b \x01(\v2\x1a.spaceone.api.core.v2.PageR\x04page\"\xef\x02\n" +
	"\fExportOption\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x14\n" +
	"\x05title\x18\x02 \x01(\tR\x05title\x12K\n" +
	"\n" +
	"query_type\x18\v \x01(\x0e2,.spaceone.api.core.v2.ExportOption.QueryTypeR\tqueryType\x12L\n" +
	"\fsearch_query\x18\f \x01(\v2'.spaceone.api.core.v2.ExportSearchQueryH\x00R\vsearchQuery\x12O\n" +
	"\ranalyze_query\x18\r \x01(\v2(.spaceone.api.core.v2.ExportAnalyzeQueryH\x00R\fanalyzeQuery\"9\n" +
	"\tQueryType\x12\x13\n" +
	"\x0fQUERY_TYPE_NONE\x10\x00\x12\n" +
	"\n" +
	"\x06SEARCH\x10\x01\x12\v\n" +
	"\aANALYZE\x10\x02B\x0e\n" +
	"\fexport_queryB;Z9github.com/cloudforet-io/api/dist/go/spaceone/api/core/v2b\x06proto3"

var (
	file_spaceone_api_core_v2_query_proto_rawDescOnce sync.Once
	file_spaceone_api_core_v2_query_proto_rawDescData []byte
)

func file_spaceone_api_core_v2_query_proto_rawDescGZIP() []byte {
	file_spaceone_api_core_v2_query_proto_rawDescOnce.Do(func() {
		file_spaceone_api_core_v2_query_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_spaceone_api_core_v2_query_proto_rawDesc), len(file_spaceone_api_core_v2_query_proto_rawDesc)))
	})
	return file_spaceone_api_core_v2_query_proto_rawDescData
}

var file_spaceone_api_core_v2_query_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_spaceone_api_core_v2_query_proto_msgTypes = make([]protoimpl.MessageInfo, 22)
var file_spaceone_api_core_v2_query_proto_goTypes = []any{
	(TimeSeriesAnalyzeQuery_Granularity)(0), // 0: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.Granularity
	(ExportOption_QueryType)(0),             // 1: spaceone.api.core.v2.ExportOption.QueryType
	(*Filter)(nil),                          // 2: spaceone.api.core.v2.Filter
	(*Sort)(nil),                            // 3: spaceone.api.core.v2.Sort
	(*Page)(nil),                            // 4: spaceone.api.core.v2.Page
	(*Unwind)(nil),                          // 5: spaceone.api.core.v2.Unwind
	(*Query)(nil),                           // 6: spaceone.api.core.v2.Query
	(*AggregateGroupKey)(nil),               // 7: spaceone.api.core.v2.AggregateGroupKey
	(*AggregateGroupSubField)(nil),          // 8: spaceone.api.core.v2.AggregateGroupSubField
	(*AggregateSubCondition)(nil),           // 9: spaceone.api.core.v2.AggregateSubCondition
	(*AggregateGroupField)(nil),             // 10: spaceone.api.core.v2.AggregateGroupField
	(*AggregateGroup)(nil),                  // 11: spaceone.api.core.v2.AggregateGroup
	(*AggregateProjectField)(nil),           // 12: spaceone.api.core.v2.AggregateProjectField
	(*AggregateProject)(nil),                // 13: spaceone.api.core.v2.AggregateProject
	(*AggregateUnwind)(nil),                 // 14: spaceone.api.core.v2.AggregateUnwind
	(*AggregateCount)(nil),                  // 15: spaceone.api.core.v2.AggregateCount
	(*StatisticsAggregate)(nil),             // 16: spaceone.api.core.v2.StatisticsAggregate
	(*StatisticsQuery)(nil),                 // 17: spaceone.api.core.v2.StatisticsQuery
	(*AnalyzeUnwind)(nil),                   // 18: spaceone.api.core.v2.AnalyzeUnwind
	(*TimeSeriesAnalyzeQuery)(nil),          // 19: spaceone.api.core.v2.TimeSeriesAnalyzeQuery
	(*AnalyzeQuery)(nil),                    // 20: spaceone.api.core.v2.AnalyzeQuery
	(*ExportSearchQuery)(nil),               // 21: spaceone.api.core.v2.ExportSearchQuery
	(*ExportAnalyzeQuery)(nil),              // 22: spaceone.api.core.v2.ExportAnalyzeQuery
	(*ExportOption)(nil),                    // 23: spaceone.api.core.v2.ExportOption
	(*_struct.Value)(nil),                   // 24: google.protobuf.Value
	(*_struct.ListValue)(nil),               // 25: google.protobuf.ListValue
	(*_struct.Struct)(nil),                  // 26: google.protobuf.Struct
}
var file_spaceone_api_core_v2_query_proto_depIdxs = []int32{
	24, // 0: spaceone.api.core.v2.Filter.value:type_name -> google.protobuf.Value
	24, // 1: spaceone.api.core.v2.Filter.v:type_name -> google.protobuf.Value
	2,  // 2: spaceone.api.core.v2.Unwind.filter:type_name -> spaceone.api.core.v2.Filter
	2,  // 3: spaceone.api.core.v2.Query.filter:type_name -> spaceone.api.core.v2.Filter
	2,  // 4: spaceone.api.core.v2.Query.filter_or:type_name -> spaceone.api.core.v2.Filter
	3,  // 5: spaceone.api.core.v2.Query.sort:type_name -> spaceone.api.core.v2.Sort
	4,  // 6: spaceone.api.core.v2.Query.page:type_name -> spaceone.api.core.v2.Page
	5,  // 7: spaceone.api.core.v2.Query.unwind:type_name -> spaceone.api.core.v2.Unwind
	24, // 8: spaceone.api.core.v2.AggregateSubCondition.value:type_name -> google.protobuf.Value
	24, // 9: spaceone.api.core.v2.AggregateSubCondition.v:type_name -> google.protobuf.Value
	8,  // 10: spaceone.api.core.v2.AggregateGroupField.fields:type_name -> spaceone.api.core.v2.AggregateGroupSubField
	9,  // 11: spaceone.api.core.v2.AggregateGroupField.conditions:type_name -> spaceone.api.core.v2.AggregateSubCondition
	7,  // 12: spaceone.api.core.v2.AggregateGroup.keys:type_name -> spaceone.api.core.v2.AggregateGroupKey
	10, // 13: spaceone.api.core.v2.AggregateGroup.fields:type_name -> spaceone.api.core.v2.AggregateGroupField
	12, // 14: spaceone.api.core.v2.AggregateProject.fields:type_name -> spaceone.api.core.v2.AggregateProjectField
	14, // 15: spaceone.api.core.v2.StatisticsAggregate.unwind:type_name -> spaceone.api.core.v2.AggregateUnwind
	11, // 16: spaceone.api.core.v2.StatisticsAggregate.group:type_name -> spaceone.api.core.v2.AggregateGroup
	15, // 17: spaceone.api.core.v2.StatisticsAggregate.count:type_name -> spaceone.api.core.v2.AggregateCount
	25, // 18: spaceone.api.core.v2.StatisticsAggregate.sort:type_name -> google.protobuf.ListValue
	13, // 19: spaceone.api.core.v2.StatisticsAggregate.project:type_name -> spaceone.api.core.v2.AggregateProject
	2,  // 20: spaceone.api.core.v2.StatisticsQuery.filter:type_name -> spaceone.api.core.v2.Filter
	2,  // 21: spaceone.api.core.v2.StatisticsQuery.filter_or:type_name -> spaceone.api.core.v2.Filter
	16, // 22: spaceone.api.core.v2.StatisticsQuery.aggregate:type_name -> spaceone.api.core.v2.StatisticsAggregate
	4,  // 23: spaceone.api.core.v2.StatisticsQuery.page:type_name -> spaceone.api.core.v2.Page
	0,  // 24: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.granularity:type_name -> spaceone.api.core.v2.TimeSeriesAnalyzeQuery.Granularity
	25, // 25: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.group_by:type_name -> google.protobuf.ListValue
	2,  // 26: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.filter:type_name -> spaceone.api.core.v2.Filter
	2,  // 27: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.filter_or:type_name -> spaceone.api.core.v2.Filter
	4,  // 28: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.page:type_name -> spaceone.api.core.v2.Page
	3,  // 29: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.sort:type_name -> spaceone.api.core.v2.Sort
	26, // 30: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.fields:type_name -> google.protobuf.Struct
	26, // 31: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.select:type_name -> google.protobuf.Struct
	18, // 32: spaceone.api.core.v2.TimeSeriesAnalyzeQuery.unwind:type_name -> spaceone.api.core.v2.AnalyzeUnwind
	25, // 33: spaceone.api.core.v2.AnalyzeQuery.group_by:type_name -> google.protobuf.ListValue
	2,  // 34: spaceone.api.core.v2.AnalyzeQuery.filter:type_name -> spaceone.api.core.v2.Filter
	2,  // 35: spaceone.api.core.v2.AnalyzeQuery.filter_or:type_name -> spaceone.api.core.v2.Filter
	4,  // 36: spaceone.api.core.v2.AnalyzeQuery.page:type_name -> spaceone.api.core.v2.Page
	3,  // 37: spaceone.api.core.v2.AnalyzeQuery.sort:type_name -> spaceone.api.core.v2.Sort
	26, // 38: spaceone.api.core.v2.AnalyzeQuery.fields:type_name -> google.protobuf.Struct
	26, // 39: spaceone.api.core.v2.AnalyzeQuery.select:type_name -> google.protobuf.Struct
	18, // 40: spaceone.api.core.v2.AnalyzeQuery.unwind:type_name -> spaceone.api.core.v2.AnalyzeUnwind
	2,  // 41: spaceone.api.core.v2.ExportSearchQuery.filter:type_name -> spaceone.api.core.v2.Filter
	2,  // 42: spaceone.api.core.v2.ExportSearchQuery.filter_or:type_name -> spaceone.api.core.v2.Filter
	3,  // 43: spaceone.api.core.v2.ExportSearchQuery.sort:type_name -> spaceone.api.core.v2.Sort
	25, // 44: spaceone.api.core.v2.ExportSearchQuery.fields:type_name -> google.protobuf.ListValue
	5,  // 45: spaceone.api.core.v2.ExportSearchQuery.unwind:type_name -> spaceone.api.core.v2.Unwind
	4,  // 46: spaceone.api.core.v2.ExportSearchQuery.page:type_name -> spaceone.api.core.v2.Page
	2,  // 47: spaceone.api.core.v2.ExportAnalyzeQuery.filter:type_name -> spaceone.api.core.v2.Filter
	2,  // 48: spaceone.api.core.v2.ExportAnalyzeQuery.filter_or:type_name -> spaceone.api.core.v2.Filter
	3,  // 49: spaceone.api.core.v2.ExportAnalyzeQuery.sort:type_name -> spaceone.api.core.v2.Sort
	25, // 50: spaceone.api.core.v2.ExportAnalyzeQuery.group_by:type_name -> google.protobuf.ListValue
	26, // 51: spaceone.api.core.v2.ExportAnalyzeQuery.fields:type_name -> google.protobuf.Struct
	26, // 52: spaceone.api.core.v2.ExportAnalyzeQuery.select:type_name -> google.protobuf.Struct
	4,  // 53: spaceone.api.core.v2.ExportAnalyzeQuery.page:type_name -> spaceone.api.core.v2.Page
	1,  // 54: spaceone.api.core.v2.ExportOption.query_type:type_name -> spaceone.api.core.v2.ExportOption.QueryType
	21, // 55: spaceone.api.core.v2.ExportOption.search_query:type_name -> spaceone.api.core.v2.ExportSearchQuery
	22, // 56: spaceone.api.core.v2.ExportOption.analyze_query:type_name -> spaceone.api.core.v2.ExportAnalyzeQuery
	57, // [57:57] is the sub-list for method output_type
	57, // [57:57] is the sub-list for method input_type
	57, // [57:57] is the sub-list for extension type_name
	57, // [57:57] is the sub-list for extension extendee
	0,  // [0:57] is the sub-list for field type_name
}

func init() { file_spaceone_api_core_v2_query_proto_init() }
func file_spaceone_api_core_v2_query_proto_init() {
	if File_spaceone_api_core_v2_query_proto != nil {
		return
	}
	file_spaceone_api_core_v2_query_proto_msgTypes[0].OneofWrappers = []any{
		(*Filter_Key)(nil),
		(*Filter_K)(nil),
		(*Filter_Value)(nil),
		(*Filter_V)(nil),
		(*Filter_Operator)(nil),
		(*Filter_O)(nil),
	}
	file_spaceone_api_core_v2_query_proto_msgTypes[5].OneofWrappers = []any{
		(*AggregateGroupKey_Key)(nil),
		(*AggregateGroupKey_K)(nil),
		(*AggregateGroupKey_Name)(nil),
		(*AggregateGroupKey_N)(nil),
	}
	file_spaceone_api_core_v2_query_proto_msgTypes[6].OneofWrappers = []any{
		(*AggregateGroupSubField_Key)(nil),
		(*AggregateGroupSubField_K)(nil),
		(*AggregateGroupSubField_Name)(nil),
		(*AggregateGroupSubField_N)(nil),
	}
	file_spaceone_api_core_v2_query_proto_msgTypes[7].OneofWrappers = []any{
		(*AggregateSubCondition_Key)(nil),
		(*AggregateSubCondition_K)(nil),
		(*AggregateSubCondition_Value)(nil),
		(*AggregateSubCondition_V)(nil),
		(*AggregateSubCondition_Operator)(nil),
		(*AggregateSubCondition_O)(nil),
	}
	file_spaceone_api_core_v2_query_proto_msgTypes[8].OneofWrappers = []any{
		(*AggregateGroupField_Key)(nil),
		(*AggregateGroupField_K)(nil),
		(*AggregateGroupField_Name)(nil),
		(*AggregateGroupField_N)(nil),
		(*AggregateGroupField_Operator)(nil),
		(*AggregateGroupField_O)(nil),
	}
	file_spaceone_api_core_v2_query_proto_msgTypes[10].OneofWrappers = []any{
		(*AggregateProjectField_Key)(nil),
		(*AggregateProjectField_K)(nil),
		(*AggregateProjectField_Name)(nil),
		(*AggregateProjectField_N)(nil),
		(*AggregateProjectField_Operator)(nil),
		(*AggregateProjectField_O)(nil),
	}
	file_spaceone_api_core_v2_query_proto_msgTypes[14].OneofWrappers = []any{
		(*StatisticsAggregate_Unwind)(nil),
		(*StatisticsAggregate_Group)(nil),
		(*StatisticsAggregate_Count)(nil),
		(*StatisticsAggregate_Sort)(nil),
		(*StatisticsAggregate_Project)(nil),
		(*StatisticsAggregate_Limit)(nil),
		(*StatisticsAggregate_Skip)(nil),
	}
	file_spaceone_api_core_v2_query_proto_msgTypes[21].OneofWrappers = []any{
		(*ExportOption_SearchQuery)(nil),
		(*ExportOption_AnalyzeQuery)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_spaceone_api_core_v2_query_proto_rawDesc), len(file_spaceone_api_core_v2_query_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   22,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_spaceone_api_core_v2_query_proto_goTypes,
		DependencyIndexes: file_spaceone_api_core_v2_query_proto_depIdxs,
		EnumInfos:         file_spaceone_api_core_v2_query_proto_enumTypes,
		MessageInfos:      file_spaceone_api_core_v2_query_proto_msgTypes,
	}.Build()
	File_spaceone_api_core_v2_query_proto = out.File
	file_spaceone_api_core_v2_query_proto_goTypes = nil
	file_spaceone_api_core_v2_query_proto_depIdxs = nil
}
