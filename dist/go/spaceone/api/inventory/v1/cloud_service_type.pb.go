// A CloudServiceType is a classification with hierarchical information of `CloudService`. A CloudServiceType provides information about which `group` a specific `Resource` belongs to and which `Services` are in it.

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        v3.6.1
// source: spaceone/api/inventory/v1/cloud_service_type.proto

package v1

import (
	v2 "github.com/cloudforet-io/api/dist/go/spaceone/api/core/v2"
	empty "github.com/golang/protobuf/ptypes/empty"
	_struct "github.com/golang/protobuf/ptypes/struct"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

//	{
//	   "name": "API-TEST",
//	   "provider": "aws",
//	   "group": "APIGateway",
//	   "service_code": "AmazonApiGateway",
//	   "is_primary": true,
//	   "is_major": true,
//	   "resource_type": "inventory.CloudService",
//	   "metadata": {},
//	   "labels": [
//	       "Networking"
//	   ],
//	   "tags": {
//	       "a": "b"
//	   }
//	}
type CreateCloudServiceTypeRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Name     string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Provider string                 `protobuf:"bytes,2,opt,name=provider,proto3" json:"provider,omitempty"`
	Group    string                 `protobuf:"bytes,3,opt,name=group,proto3" json:"group,omitempty"`
	// +optional
	ServiceCode string `protobuf:"bytes,4,opt,name=service_code,json=serviceCode,proto3" json:"service_code,omitempty"`
	// +optional
	IsPrimary bool `protobuf:"varint,5,opt,name=is_primary,json=isPrimary,proto3" json:"is_primary,omitempty"`
	// +optional
	IsMajor bool `protobuf:"varint,6,opt,name=is_major,json=isMajor,proto3" json:"is_major,omitempty"`
	// +optional
	ResourceType string `protobuf:"bytes,7,opt,name=resource_type,json=resourceType,proto3" json:"resource_type,omitempty"`
	// +optional
	Metadata *_struct.Struct `protobuf:"bytes,11,opt,name=metadata,proto3" json:"metadata,omitempty"`
	// +optional
	Labels *_struct.ListValue `protobuf:"bytes,12,opt,name=labels,proto3" json:"labels,omitempty"`
	// +optional
	Tags          *_struct.Struct `protobuf:"bytes,13,opt,name=tags,proto3" json:"tags,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateCloudServiceTypeRequest) Reset() {
	*x = CreateCloudServiceTypeRequest{}
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateCloudServiceTypeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateCloudServiceTypeRequest) ProtoMessage() {}

func (x *CreateCloudServiceTypeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateCloudServiceTypeRequest.ProtoReflect.Descriptor instead.
func (*CreateCloudServiceTypeRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescGZIP(), []int{0}
}

func (x *CreateCloudServiceTypeRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateCloudServiceTypeRequest) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *CreateCloudServiceTypeRequest) GetGroup() string {
	if x != nil {
		return x.Group
	}
	return ""
}

func (x *CreateCloudServiceTypeRequest) GetServiceCode() string {
	if x != nil {
		return x.ServiceCode
	}
	return ""
}

func (x *CreateCloudServiceTypeRequest) GetIsPrimary() bool {
	if x != nil {
		return x.IsPrimary
	}
	return false
}

func (x *CreateCloudServiceTypeRequest) GetIsMajor() bool {
	if x != nil {
		return x.IsMajor
	}
	return false
}

func (x *CreateCloudServiceTypeRequest) GetResourceType() string {
	if x != nil {
		return x.ResourceType
	}
	return ""
}

func (x *CreateCloudServiceTypeRequest) GetMetadata() *_struct.Struct {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *CreateCloudServiceTypeRequest) GetLabels() *_struct.ListValue {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *CreateCloudServiceTypeRequest) GetTags() *_struct.Struct {
	if x != nil {
		return x.Tags
	}
	return nil
}

//	{
//	   "cloud_service_type_id": "cloud-svc-type-27dd73ac89f8",
//	   "service_code": "AmazonApi",
//	   "metadata": {},
//	   "labels": [
//	       "Networking2"
//	   ],
//	   "tags": {
//	       "b": "c"
//	   }
//	}
type UpdateCloudServiceTypeRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	CloudServiceTypeId string                 `protobuf:"bytes,1,opt,name=cloud_service_type_id,json=cloudServiceTypeId,proto3" json:"cloud_service_type_id,omitempty"`
	// +optional
	ServiceCode string `protobuf:"bytes,2,opt,name=service_code,json=serviceCode,proto3" json:"service_code,omitempty"`
	// +optional
	IsPrimary bool `protobuf:"varint,3,opt,name=is_primary,json=isPrimary,proto3" json:"is_primary,omitempty"`
	// +optional
	IsMajor bool `protobuf:"varint,4,opt,name=is_major,json=isMajor,proto3" json:"is_major,omitempty"`
	// +optional
	ResourceType string `protobuf:"bytes,5,opt,name=resource_type,json=resourceType,proto3" json:"resource_type,omitempty"`
	// +optional
	Metadata *_struct.Struct `protobuf:"bytes,11,opt,name=metadata,proto3" json:"metadata,omitempty"`
	// +optional
	Labels *_struct.ListValue `protobuf:"bytes,12,opt,name=labels,proto3" json:"labels,omitempty"`
	// +optional
	Tags          *_struct.Struct `protobuf:"bytes,13,opt,name=tags,proto3" json:"tags,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateCloudServiceTypeRequest) Reset() {
	*x = UpdateCloudServiceTypeRequest{}
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateCloudServiceTypeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateCloudServiceTypeRequest) ProtoMessage() {}

func (x *UpdateCloudServiceTypeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateCloudServiceTypeRequest.ProtoReflect.Descriptor instead.
func (*UpdateCloudServiceTypeRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescGZIP(), []int{1}
}

func (x *UpdateCloudServiceTypeRequest) GetCloudServiceTypeId() string {
	if x != nil {
		return x.CloudServiceTypeId
	}
	return ""
}

func (x *UpdateCloudServiceTypeRequest) GetServiceCode() string {
	if x != nil {
		return x.ServiceCode
	}
	return ""
}

func (x *UpdateCloudServiceTypeRequest) GetIsPrimary() bool {
	if x != nil {
		return x.IsPrimary
	}
	return false
}

func (x *UpdateCloudServiceTypeRequest) GetIsMajor() bool {
	if x != nil {
		return x.IsMajor
	}
	return false
}

func (x *UpdateCloudServiceTypeRequest) GetResourceType() string {
	if x != nil {
		return x.ResourceType
	}
	return ""
}

func (x *UpdateCloudServiceTypeRequest) GetMetadata() *_struct.Struct {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *UpdateCloudServiceTypeRequest) GetLabels() *_struct.ListValue {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *UpdateCloudServiceTypeRequest) GetTags() *_struct.Struct {
	if x != nil {
		return x.Tags
	}
	return nil
}

//	{
//	   "cloud_service_type_id": "cloud-svc-type-27dd73ac89f8"
//	}
type CloudServiceTypeRequest struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	CloudServiceTypeId string                 `protobuf:"bytes,1,opt,name=cloud_service_type_id,json=cloudServiceTypeId,proto3" json:"cloud_service_type_id,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *CloudServiceTypeRequest) Reset() {
	*x = CloudServiceTypeRequest{}
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CloudServiceTypeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloudServiceTypeRequest) ProtoMessage() {}

func (x *CloudServiceTypeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloudServiceTypeRequest.ProtoReflect.Descriptor instead.
func (*CloudServiceTypeRequest) Descriptor() ([]byte, []int) {
	return file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescGZIP(), []int{2}
}

func (x *CloudServiceTypeRequest) GetCloudServiceTypeId() string {
	if x != nil {
		return x.CloudServiceTypeId
	}
	return ""
}

//	{
//	   "query": {
//	       "filter": [
//	           {
//	               "key": "provider",
//	               "value": "aws",
//	               "operator": "eq"
//	           }
//	       ]
//	   }
//	}
type CloudServiceTypeQuery struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// +optional
	Query *v2.Query `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	// +optional
	CloudServiceTypeId string `protobuf:"bytes,2,opt,name=cloud_service_type_id,json=cloudServiceTypeId,proto3" json:"cloud_service_type_id,omitempty"`
	// +optional
	Name string `protobuf:"bytes,3,opt,name=name,proto3" json:"name,omitempty"`
	// +optional
	Provider string `protobuf:"bytes,4,opt,name=provider,proto3" json:"provider,omitempty"`
	// +optional
	Group string `protobuf:"bytes,5,opt,name=group,proto3" json:"group,omitempty"`
	// +optional
	CloudServiceTypeKey string `protobuf:"bytes,6,opt,name=cloud_service_type_key,json=cloudServiceTypeKey,proto3" json:"cloud_service_type_key,omitempty"`
	// +optional
	ServiceCode string `protobuf:"bytes,7,opt,name=service_code,json=serviceCode,proto3" json:"service_code,omitempty"`
	// +optional
	IsPrimary bool `protobuf:"varint,8,opt,name=is_primary,json=isPrimary,proto3" json:"is_primary,omitempty"`
	// +optional
	IsMajor bool `protobuf:"varint,9,opt,name=is_major,json=isMajor,proto3" json:"is_major,omitempty"`
	// +optional
	ResourceType string `protobuf:"bytes,10,opt,name=resource_type,json=resourceType,proto3" json:"resource_type,omitempty"`
	// +optional
	WorkspaceId   string `protobuf:"bytes,21,opt,name=workspace_id,json=workspaceId,proto3" json:"workspace_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CloudServiceTypeQuery) Reset() {
	*x = CloudServiceTypeQuery{}
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CloudServiceTypeQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloudServiceTypeQuery) ProtoMessage() {}

func (x *CloudServiceTypeQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloudServiceTypeQuery.ProtoReflect.Descriptor instead.
func (*CloudServiceTypeQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescGZIP(), []int{3}
}

func (x *CloudServiceTypeQuery) GetQuery() *v2.Query {
	if x != nil {
		return x.Query
	}
	return nil
}

func (x *CloudServiceTypeQuery) GetCloudServiceTypeId() string {
	if x != nil {
		return x.CloudServiceTypeId
	}
	return ""
}

func (x *CloudServiceTypeQuery) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CloudServiceTypeQuery) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *CloudServiceTypeQuery) GetGroup() string {
	if x != nil {
		return x.Group
	}
	return ""
}

func (x *CloudServiceTypeQuery) GetCloudServiceTypeKey() string {
	if x != nil {
		return x.CloudServiceTypeKey
	}
	return ""
}

func (x *CloudServiceTypeQuery) GetServiceCode() string {
	if x != nil {
		return x.ServiceCode
	}
	return ""
}

func (x *CloudServiceTypeQuery) GetIsPrimary() bool {
	if x != nil {
		return x.IsPrimary
	}
	return false
}

func (x *CloudServiceTypeQuery) GetIsMajor() bool {
	if x != nil {
		return x.IsMajor
	}
	return false
}

func (x *CloudServiceTypeQuery) GetResourceType() string {
	if x != nil {
		return x.ResourceType
	}
	return ""
}

func (x *CloudServiceTypeQuery) GetWorkspaceId() string {
	if x != nil {
		return x.WorkspaceId
	}
	return ""
}

//	{
//	   "cloud_service_type_id": "cloud-svc-type-27dd73ac89f8",
//	   "name": "API-TEST",
//	   "provider": "aws",
//	   "group": "APIGateway",
//	   "cloud_service_type_key": "aws.APIGateway.API-TEST",
//	   "service_code": "AmazonApiGateway",
//	   "is_primary": true,
//	   "is_major": true,
//	   "resource_type": "inventory.CloudService",
//	   "metadata": {},
//	   "tags": {
//	       "a": "b"
//	   },
//	   "labels": [
//	       "Networking"
//	   ],
//	   "domain_id": "domain-58010aa2e451",
//	   "workspace_id": "workspace-abcde12345",
//	   "created_at": "2022-06-22T01:38:16.301Z",
//	   "updated_at": "2022-06-22T01:38:16.301Z"
//	}
type CloudServiceTypeInfo struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	CloudServiceTypeId  string                 `protobuf:"bytes,1,opt,name=cloud_service_type_id,json=cloudServiceTypeId,proto3" json:"cloud_service_type_id,omitempty"`
	Name                string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Provider            string                 `protobuf:"bytes,3,opt,name=provider,proto3" json:"provider,omitempty"`
	Group               string                 `protobuf:"bytes,4,opt,name=group,proto3" json:"group,omitempty"`
	CloudServiceTypeKey string                 `protobuf:"bytes,5,opt,name=cloud_service_type_key,json=cloudServiceTypeKey,proto3" json:"cloud_service_type_key,omitempty"`
	ServiceCode         string                 `protobuf:"bytes,6,opt,name=service_code,json=serviceCode,proto3" json:"service_code,omitempty"`
	IsPrimary           bool                   `protobuf:"varint,7,opt,name=is_primary,json=isPrimary,proto3" json:"is_primary,omitempty"`
	IsMajor             bool                   `protobuf:"varint,8,opt,name=is_major,json=isMajor,proto3" json:"is_major,omitempty"`
	ResourceType        string                 `protobuf:"bytes,9,opt,name=resource_type,json=resourceType,proto3" json:"resource_type,omitempty"`
	Metadata            *_struct.Struct        `protobuf:"bytes,11,opt,name=metadata,proto3" json:"metadata,omitempty"`
	Tags                *_struct.Struct        `protobuf:"bytes,12,opt,name=tags,proto3" json:"tags,omitempty"`
	Labels              *_struct.ListValue     `protobuf:"bytes,13,opt,name=labels,proto3" json:"labels,omitempty"`
	DomainId            string                 `protobuf:"bytes,21,opt,name=domain_id,json=domainId,proto3" json:"domain_id,omitempty"`
	WorkspaceId         string                 `protobuf:"bytes,22,opt,name=workspace_id,json=workspaceId,proto3" json:"workspace_id,omitempty"`
	CreatedAt           string                 `protobuf:"bytes,31,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	UpdatedAt           string                 `protobuf:"bytes,32,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *CloudServiceTypeInfo) Reset() {
	*x = CloudServiceTypeInfo{}
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CloudServiceTypeInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloudServiceTypeInfo) ProtoMessage() {}

func (x *CloudServiceTypeInfo) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloudServiceTypeInfo.ProtoReflect.Descriptor instead.
func (*CloudServiceTypeInfo) Descriptor() ([]byte, []int) {
	return file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescGZIP(), []int{4}
}

func (x *CloudServiceTypeInfo) GetCloudServiceTypeId() string {
	if x != nil {
		return x.CloudServiceTypeId
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetGroup() string {
	if x != nil {
		return x.Group
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetCloudServiceTypeKey() string {
	if x != nil {
		return x.CloudServiceTypeKey
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetServiceCode() string {
	if x != nil {
		return x.ServiceCode
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetIsPrimary() bool {
	if x != nil {
		return x.IsPrimary
	}
	return false
}

func (x *CloudServiceTypeInfo) GetIsMajor() bool {
	if x != nil {
		return x.IsMajor
	}
	return false
}

func (x *CloudServiceTypeInfo) GetResourceType() string {
	if x != nil {
		return x.ResourceType
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetMetadata() *_struct.Struct {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *CloudServiceTypeInfo) GetTags() *_struct.Struct {
	if x != nil {
		return x.Tags
	}
	return nil
}

func (x *CloudServiceTypeInfo) GetLabels() *_struct.ListValue {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *CloudServiceTypeInfo) GetDomainId() string {
	if x != nil {
		return x.DomainId
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetWorkspaceId() string {
	if x != nil {
		return x.WorkspaceId
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

func (x *CloudServiceTypeInfo) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

//	{
//	   "results": [
//	       {
//	           "cloud_service_type_id": "cloud-svc-type-7e1c113b39ff",
//	           "name": "API",
//	           "provider": "aws",
//	           "group": "APIGateway",
//	           "cloud_service_type_key": "aws.APIGateway.API",
//	           "service_code": "AmazonApiGateway",
//	           "is_primary": true,
//	           "is_major": true,
//	           "resource_type": "inventory.CloudService",
//	           "metadata": {
//	           },
//	           "tags": {
//	               "spaceone:icon": "https://spaceone.s3.ap-northeast-2.amazonaws.com/console-assets/icons/cloud-services/aws/Amazon-API-Gateway.svg"
//	           },
//	           "labels": [
//	               "Networking"
//	           ],
//	           "domain_id": "domain-58010aa2e451",
//	           "workspace_id": "workspace-abcde12345",
//	           "created_at": "2021-06-03T02:29:32.690Z",
//	           "updated_at": "2022-06-22T00:04:45.477Z"
//	       },
//	       {
//	           "cloud_service_type_id": "cloud-svc-type-64a0de601371",
//	           "name": "Certificate",
//	           "provider": "aws",
//	           "group": "CertificateManager",
//	           "cloud_service_type_key": "aws.CertificateManager.Certificate",
//	           "service_code": "AWSCertificateManager",
//	           "is_primary": true,
//	           "resource_type": "inventory.CloudService",
//	           "metadata": {
//	           },
//	           "tags": {
//	               "spaceone:icon": "https://spaceone.s3.ap-northeast-2.amazonaws.com/console-assets/icons/cloud-services/aws/AWS-Certificate-Manager.svg"
//	           },
//	           "labels": [
//	               "Security"
//	           ],
//	           "domain_id": "domain-58010aa2e451",
//	           "workspace_id": "workspace-abcde12345",
//	           "created_at": "2021-06-03T02:29:53.052Z",
//	           "updated_at": "2022-06-22T00:05:41.252Z"
//	       }
//	   ],
//	   "total_count": 2
//	}
type CloudServiceTypesInfo struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Results       []*CloudServiceTypeInfo `protobuf:"bytes,1,rep,name=results,proto3" json:"results,omitempty"`
	TotalCount    int32                   `protobuf:"varint,2,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CloudServiceTypesInfo) Reset() {
	*x = CloudServiceTypesInfo{}
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CloudServiceTypesInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloudServiceTypesInfo) ProtoMessage() {}

func (x *CloudServiceTypesInfo) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloudServiceTypesInfo.ProtoReflect.Descriptor instead.
func (*CloudServiceTypesInfo) Descriptor() ([]byte, []int) {
	return file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescGZIP(), []int{5}
}

func (x *CloudServiceTypesInfo) GetResults() []*CloudServiceTypeInfo {
	if x != nil {
		return x.Results
	}
	return nil
}

func (x *CloudServiceTypesInfo) GetTotalCount() int32 {
	if x != nil {
		return x.TotalCount
	}
	return 0
}

type CloudServiceTypeStatQuery struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Query         *v2.StatisticsQuery    `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	DomainId      string                 `protobuf:"bytes,2,opt,name=domain_id,json=domainId,proto3" json:"domain_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CloudServiceTypeStatQuery) Reset() {
	*x = CloudServiceTypeStatQuery{}
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CloudServiceTypeStatQuery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloudServiceTypeStatQuery) ProtoMessage() {}

func (x *CloudServiceTypeStatQuery) ProtoReflect() protoreflect.Message {
	mi := &file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloudServiceTypeStatQuery.ProtoReflect.Descriptor instead.
func (*CloudServiceTypeStatQuery) Descriptor() ([]byte, []int) {
	return file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescGZIP(), []int{6}
}

func (x *CloudServiceTypeStatQuery) GetQuery() *v2.StatisticsQuery {
	if x != nil {
		return x.Query
	}
	return nil
}

func (x *CloudServiceTypeStatQuery) GetDomainId() string {
	if x != nil {
		return x.DomainId
	}
	return ""
}

var File_spaceone_api_inventory_v1_cloud_service_type_proto protoreflect.FileDescriptor

const file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDesc = "" +
	"\n" +
	"2spaceone/api/inventory/v1/cloud_service_type.proto\x12\x19spaceone.api.inventory.v1\x1a\x1bgoogle/protobuf/empty.proto\x1a\x1cgoogle/protobuf/struct.proto\x1a\x1cgoogle/api/annotations.proto\x1a spaceone/api/core/v2/query.proto\"\xfd\x02\n" +
	"\x1dCreateCloudServiceTypeRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x1a\n" +
	"\bprovider\x18\x02 \x01(\tR\bprovider\x12\x14\n" +
	"\x05group\x18\x03 \x01(\tR\x05group\x12!\n" +
	"\fservice_code\x18\x04 \x01(\tR\vserviceCode\x12\x1d\n" +
	"\n" +
	"is_primary\x18\x05 \x01(\bR\tisPrimary\x12\x19\n" +
	"\bis_major\x18\x06 \x01(\bR\aisMajor\x12#\n" +
	"\rresource_type\x18\a \x01(\tR\fresourceType\x123\n" +
	"\bmetadata\x18\v \x01(\v2\x17.google.protobuf.StructR\bmetadata\x122\n" +
	"\x06labels\x18\f \x01(\v2\x1a.google.protobuf.ListValueR\x06labels\x12+\n" +
	"\x04tags\x18\r \x01(\v2\x17.google.protobuf.StructR\x04tags\"\xea\x02\n" +
	"\x1dUpdateCloudServiceTypeRequest\x121\n" +
	"\x15cloud_service_type_id\x18\x01 \x01(\tR\x12cloudServiceTypeId\x12!\n" +
	"\fservice_code\x18\x02 \x01(\tR\vserviceCode\x12\x1d\n" +
	"\n" +
	"is_primary\x18\x03 \x01(\bR\tisPrimary\x12\x19\n" +
	"\bis_major\x18\x04 \x01(\bR\aisMajor\x12#\n" +
	"\rresource_type\x18\x05 \x01(\tR\fresourceType\x123\n" +
	"\bmetadata\x18\v \x01(\v2\x17.google.protobuf.StructR\bmetadata\x122\n" +
	"\x06labels\x18\f \x01(\v2\x1a.google.protobuf.ListValueR\x06labels\x12+\n" +
	"\x04tags\x18\r \x01(\v2\x17.google.protobuf.StructR\x04tags\"L\n" +
	"\x17CloudServiceTypeRequest\x121\n" +
	"\x15cloud_service_type_id\x18\x01 \x01(\tR\x12cloudServiceTypeId\"\x9d\x03\n" +
	"\x15CloudServiceTypeQuery\x121\n" +
	"\x05query\x18\x01 \x01(\v2\x1b.spaceone.api.core.v2.QueryR\x05query\x121\n" +
	"\x15cloud_service_type_id\x18\x02 \x01(\tR\x12cloudServiceTypeId\x12\x12\n" +
	"\x04name\x18\x03 \x01(\tR\x04name\x12\x1a\n" +
	"\bprovider\x18\x04 \x01(\tR\bprovider\x12\x14\n" +
	"\x05group\x18\x05 \x01(\tR\x05group\x123\n" +
	"\x16cloud_service_type_key\x18\x06 \x01(\tR\x13cloudServiceTypeKey\x12!\n" +
	"\fservice_code\x18\a \x01(\tR\vserviceCode\x12\x1d\n" +
	"\n" +
	"is_primary\x18\b \x01(\bR\tisPrimary\x12\x19\n" +
	"\bis_major\x18\t \x01(\bR\aisMajor\x12#\n" +
	"\rresource_type\x18\n" +
	" \x01(\tR\fresourceType\x12!\n" +
	"\fworkspace_id\x18\x15 \x01(\tR\vworkspaceId\"\xda\x04\n" +
	"\x14CloudServiceTypeInfo\x121\n" +
	"\x15cloud_service_type_id\x18\x01 \x01(\tR\x12cloudServiceTypeId\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x1a\n" +
	"\bprovider\x18\x03 \x01(\tR\bprovider\x12\x14\n" +
	"\x05group\x18\x04 \x01(\tR\x05group\x123\n" +
	"\x16cloud_service_type_key\x18\x05 \x01(\tR\x13cloudServiceTypeKey\x12!\n" +
	"\fservice_code\x18\x06 \x01(\tR\vserviceCode\x12\x1d\n" +
	"\n" +
	"is_primary\x18\a \x01(\bR\tisPrimary\x12\x19\n" +
	"\bis_major\x18\b \x01(\bR\aisMajor\x12#\n" +
	"\rresource_type\x18\t \x01(\tR\fresourceType\x123\n" +
	"\bmetadata\x18\v \x01(\v2\x17.google.protobuf.StructR\bmetadata\x12+\n" +
	"\x04tags\x18\f \x01(\v2\x17.google.protobuf.StructR\x04tags\x122\n" +
	"\x06labels\x18\r \x01(\v2\x1a.google.protobuf.ListValueR\x06labels\x12\x1b\n" +
	"\tdomain_id\x18\x15 \x01(\tR\bdomainId\x12!\n" +
	"\fworkspace_id\x18\x16 \x01(\tR\vworkspaceId\x12\x1d\n" +
	"\n" +
	"created_at\x18\x1f \x01(\tR\tcreatedAt\x12\x1d\n" +
	"\n" +
	"updated_at\x18  \x01(\tR\tupdatedAt\"\x83\x01\n" +
	"\x15CloudServiceTypesInfo\x12I\n" +
	"\aresults\x18\x01 \x03(\v2/.spaceone.api.inventory.v1.CloudServiceTypeInfoR\aresults\x12\x1f\n" +
	"\vtotal_count\x18\x02 \x01(\x05R\n" +
	"totalCount\"u\n" +
	"\x19CloudServiceTypeStatQuery\x12;\n" +
	"\x05query\x18\x01 \x01(\v2%.spaceone.api.core.v2.StatisticsQueryR\x05query\x12\x1b\n" +
	"\tdomain_id\x18\x02 \x01(\tR\bdomainId2\xb8\a\n" +
	"\x10CloudServiceType\x12\xa7\x01\n" +
	"\x06create\x128.spaceone.api.inventory.v1.CreateCloudServiceTypeRequest\x1a/.spaceone.api.inventory.v1.CloudServiceTypeInfo\"2\x82\xd3\xe4\x93\x02,:\x01*\"'/inventory/v1/cloud-service-type/create\x12\xa7\x01\n" +
	"\x06update\x128.spaceone.api.inventory.v1.UpdateCloudServiceTypeRequest\x1a/.spaceone.api.inventory.v1.CloudServiceTypeInfo\"2\x82\xd3\xe4\x93\x02,:\x01*\"'/inventory/v1/cloud-service-type/update\x12\x88\x01\n" +
	"\x06delete\x122.spaceone.api.inventory.v1.CloudServiceTypeRequest\x1a\x16.google.protobuf.Empty\"2\x82\xd3\xe4\x93\x02,:\x01*\"'/inventory/v1/cloud-service-type/delete\x12\x9b\x01\n" +
	"\x03get\x122.spaceone.api.inventory.v1.CloudServiceTypeRequest\x1a/.spaceone.api.inventory.v1.CloudServiceTypeInfo\"/\x82\xd3\xe4\x93\x02):\x01*\"$/inventory/v1/cloud-service-type/get\x12\x9c\x01\n" +
	"\x04list\x120.spaceone.api.inventory.v1.CloudServiceTypeQuery\x1a0.spaceone.api.inventory.v1.CloudServiceTypesInfo\"0\x82\xd3\xe4\x93\x02*:\x01*\"%/inventory/v1/cloud-service-type/list\x12\x87\x01\n" +
	"\x04stat\x124.spaceone.api.inventory.v1.CloudServiceTypeStatQuery\x1a\x17.google.protobuf.Struct\"0\x82\xd3\xe4\x93\x02*:\x01*\"%/inventory/v1/cloud-service-type/statB@Z>github.com/cloudforet-io/api/dist/go/spaceone/api/inventory/v1b\x06proto3"

var (
	file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescOnce sync.Once
	file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescData []byte
)

func file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescGZIP() []byte {
	file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescOnce.Do(func() {
		file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDesc), len(file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDesc)))
	})
	return file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDescData
}

var file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes = make([]protoimpl.MessageInfo, 7)
var file_spaceone_api_inventory_v1_cloud_service_type_proto_goTypes = []any{
	(*CreateCloudServiceTypeRequest)(nil), // 0: spaceone.api.inventory.v1.CreateCloudServiceTypeRequest
	(*UpdateCloudServiceTypeRequest)(nil), // 1: spaceone.api.inventory.v1.UpdateCloudServiceTypeRequest
	(*CloudServiceTypeRequest)(nil),       // 2: spaceone.api.inventory.v1.CloudServiceTypeRequest
	(*CloudServiceTypeQuery)(nil),         // 3: spaceone.api.inventory.v1.CloudServiceTypeQuery
	(*CloudServiceTypeInfo)(nil),          // 4: spaceone.api.inventory.v1.CloudServiceTypeInfo
	(*CloudServiceTypesInfo)(nil),         // 5: spaceone.api.inventory.v1.CloudServiceTypesInfo
	(*CloudServiceTypeStatQuery)(nil),     // 6: spaceone.api.inventory.v1.CloudServiceTypeStatQuery
	(*_struct.Struct)(nil),                // 7: google.protobuf.Struct
	(*_struct.ListValue)(nil),             // 8: google.protobuf.ListValue
	(*v2.Query)(nil),                      // 9: spaceone.api.core.v2.Query
	(*v2.StatisticsQuery)(nil),            // 10: spaceone.api.core.v2.StatisticsQuery
	(*empty.Empty)(nil),                   // 11: google.protobuf.Empty
}
var file_spaceone_api_inventory_v1_cloud_service_type_proto_depIdxs = []int32{
	7,  // 0: spaceone.api.inventory.v1.CreateCloudServiceTypeRequest.metadata:type_name -> google.protobuf.Struct
	8,  // 1: spaceone.api.inventory.v1.CreateCloudServiceTypeRequest.labels:type_name -> google.protobuf.ListValue
	7,  // 2: spaceone.api.inventory.v1.CreateCloudServiceTypeRequest.tags:type_name -> google.protobuf.Struct
	7,  // 3: spaceone.api.inventory.v1.UpdateCloudServiceTypeRequest.metadata:type_name -> google.protobuf.Struct
	8,  // 4: spaceone.api.inventory.v1.UpdateCloudServiceTypeRequest.labels:type_name -> google.protobuf.ListValue
	7,  // 5: spaceone.api.inventory.v1.UpdateCloudServiceTypeRequest.tags:type_name -> google.protobuf.Struct
	9,  // 6: spaceone.api.inventory.v1.CloudServiceTypeQuery.query:type_name -> spaceone.api.core.v2.Query
	7,  // 7: spaceone.api.inventory.v1.CloudServiceTypeInfo.metadata:type_name -> google.protobuf.Struct
	7,  // 8: spaceone.api.inventory.v1.CloudServiceTypeInfo.tags:type_name -> google.protobuf.Struct
	8,  // 9: spaceone.api.inventory.v1.CloudServiceTypeInfo.labels:type_name -> google.protobuf.ListValue
	4,  // 10: spaceone.api.inventory.v1.CloudServiceTypesInfo.results:type_name -> spaceone.api.inventory.v1.CloudServiceTypeInfo
	10, // 11: spaceone.api.inventory.v1.CloudServiceTypeStatQuery.query:type_name -> spaceone.api.core.v2.StatisticsQuery
	0,  // 12: spaceone.api.inventory.v1.CloudServiceType.create:input_type -> spaceone.api.inventory.v1.CreateCloudServiceTypeRequest
	1,  // 13: spaceone.api.inventory.v1.CloudServiceType.update:input_type -> spaceone.api.inventory.v1.UpdateCloudServiceTypeRequest
	2,  // 14: spaceone.api.inventory.v1.CloudServiceType.delete:input_type -> spaceone.api.inventory.v1.CloudServiceTypeRequest
	2,  // 15: spaceone.api.inventory.v1.CloudServiceType.get:input_type -> spaceone.api.inventory.v1.CloudServiceTypeRequest
	3,  // 16: spaceone.api.inventory.v1.CloudServiceType.list:input_type -> spaceone.api.inventory.v1.CloudServiceTypeQuery
	6,  // 17: spaceone.api.inventory.v1.CloudServiceType.stat:input_type -> spaceone.api.inventory.v1.CloudServiceTypeStatQuery
	4,  // 18: spaceone.api.inventory.v1.CloudServiceType.create:output_type -> spaceone.api.inventory.v1.CloudServiceTypeInfo
	4,  // 19: spaceone.api.inventory.v1.CloudServiceType.update:output_type -> spaceone.api.inventory.v1.CloudServiceTypeInfo
	11, // 20: spaceone.api.inventory.v1.CloudServiceType.delete:output_type -> google.protobuf.Empty
	4,  // 21: spaceone.api.inventory.v1.CloudServiceType.get:output_type -> spaceone.api.inventory.v1.CloudServiceTypeInfo
	5,  // 22: spaceone.api.inventory.v1.CloudServiceType.list:output_type -> spaceone.api.inventory.v1.CloudServiceTypesInfo
	7,  // 23: spaceone.api.inventory.v1.CloudServiceType.stat:output_type -> google.protobuf.Struct
	18, // [18:24] is the sub-list for method output_type
	12, // [12:18] is the sub-list for method input_type
	12, // [12:12] is the sub-list for extension type_name
	12, // [12:12] is the sub-list for extension extendee
	0,  // [0:12] is the sub-list for field type_name
}

func init() { file_spaceone_api_inventory_v1_cloud_service_type_proto_init() }
func file_spaceone_api_inventory_v1_cloud_service_type_proto_init() {
	if File_spaceone_api_inventory_v1_cloud_service_type_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDesc), len(file_spaceone_api_inventory_v1_cloud_service_type_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   7,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_spaceone_api_inventory_v1_cloud_service_type_proto_goTypes,
		DependencyIndexes: file_spaceone_api_inventory_v1_cloud_service_type_proto_depIdxs,
		MessageInfos:      file_spaceone_api_inventory_v1_cloud_service_type_proto_msgTypes,
	}.Build()
	File_spaceone_api_inventory_v1_cloud_service_type_proto = out.File
	file_spaceone_api_inventory_v1_cloud_service_type_proto_goTypes = nil
	file_spaceone_api_inventory_v1_cloud_service_type_proto_depIdxs = nil
}
