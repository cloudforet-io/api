...
type CollectorClient interface {
	...
	Collect(ctx context.Context, in *CollectRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ResourceInfo], error)
}
...
func (c *collectorClient) Collect(ctx context.Context, in *CollectRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ResourceInfo], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Collector_ServiceDesc.Streams[0], Collector_Collect_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[CollectRequest, ResourceInfo]{ClientStream: stream}
...
type Collector_CollectClient = grpc.ServerStreamingClient[ResourceInfo]
...
type CollectorServer interface {
	...
	// Collects data of external infrastructure resources by a specific Collector.
	Collect(*CollectRequest, grpc.ServerStreamingServer[ResourceInfo]) error
...
func (UnimplementedCollectorServer) Collect(*CollectRequest, grpc.ServerStreamingServer[ResourceInfo]) error {
...
func _Collector_Collect_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(CollectorServer).Collect(m, &grpc.GenericServerStream[CollectRequest, ResourceInfo]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Collector_CollectServer = grpc.ServerStreamingServer[ResourceInfo]
